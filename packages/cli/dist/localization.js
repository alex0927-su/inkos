...
function normalizeCliLanguageTag(value) {
    if (!value) {
        return undefined;
    }
...
export function resolveCliLanguage(language, env = process.env) {
...
export function formatBookCreateCreating(language, title, genre, platform) {
    return localize(language, {
        zh: `创建书籍 "${title}"（${genre} / ${platform}）...`,
        en: `Creating book "${title}" (${genre} / ${platform})...`,
    });
}
export function formatBookCreateCreated(language, bookId) {
    return localize(language, {
        zh: `已创建书籍：${bookId}`,
        en: `Book created: ${bookId}`,
    });
}
export function formatBookCreateLocation(language, bookId) {
    return localize(language, {
        zh: `  位置：books/${bookId}/`,
        en: `  Location: books/${bookId}/`,
    });
}
export function formatBookCreateFoundationReady(language) {
    return localize(language, {
        zh: "  故事圣经、大纲和书籍规则已生成。",
        en: "  Story bible, outline, book rules generated.",
    });
}
export function formatBookCreateNextStep(language, bookId) {
    return localize(language, {
        zh: `下一步：inkos write next ${bookId}`,
        en: `Next: inkos write next ${bookId}`,
    });
}
export function formatWriteNextProgress(language, current, total, bookId) {
    return localize(language, {
        zh: `[${current}/${total}] 为「${bookId}」撰写章节...`,
        en: `[${current}/${total}] Writing chapter for "${bookId}"...`,
    });
}
export function formatWriteNextResultLines(language, result) {
...
export function formatWriteNextComplete(language) {
    return localize(language, {
        zh: "完成。",
        en: "Done.",
    });
}
export function formatAutoWriteStart(language, bookId, startChapter, targetChapter) {
    return localize(language, {
...
export function formatAutoWriteAlreadyComplete(language, bookId, writtenChapters, targetChapter) {
    return localize(language, {
...
export function formatNotifyCommandTitle(language, action, bookName, succeeded) {
...
export function formatNotifyBatchWriteBody(language, chapters) {
...
export function formatNotifyAuditBody(language, result) {
...
export function formatNotifyReviseBody(language, result) {
    if (!result.applied) {
        return localize(language, {
...
export function formatNotifyFailureBody(language, error) {
...
export function formatImportChaptersDiscovery(language, chapterCount, bookId) {
    return localize(language, {
        zh: `发现 ${chapterCount} 章，准备导入到「${bookId}」。`,
        en: `Found ${chapterCount} chapters to import into "${bookId}".`,
    });
}
export function formatImportChaptersResume(language, resumeFrom) {
    return localize(language, {
        zh: `从第 ${resumeFrom} 章继续导入。`,
        en: `Resuming from chapter ${resumeFrom}.`,
    });
}
export function formatImportChaptersComplete(language, result) {
...
export function formatImportCanonStart(language, parentBookId, targetBookId) {
    return localize(language, {
        zh: `把 "${parentBookId}" 的正典导入到 "${targetBookId}"...`,
        en: `Importing canon from "${parentBookId}" into "${targetBookId}"...`,
    });
}
export function formatImportCanonComplete(language) {
    return [
        localize(language, {
...
export function formatListModelsEmpty(language, service) {
    return localize(language, {
        zh: `${service} 没有可用模型（可能需要 --api-key 和 --base-url）`,
        en: `No models available for ${service} (you may need --api-key and --base-url)`,
    });
}
export function formatListModelsHeader(language, service, count) {
    return localize(language, {
        zh: `${service}：${count} 个模型`,
        en: `${service}: ${count} model(s)`,
    });
}
export function formatDoctorHintQuota(language) {
    return localize(language, {
...
export function formatDoctorHintOpenAiProbeExhausted(language) {
    return localize(language, {
...
export function formatDoctorHintBaseUrl(language) {
    return localize(language, {
...
export function formatDoctorHintStreamRequirement(language) {
    return localize(language, {
...
export function formatDoctorHintModelName(language) {
    return localize(language, {
        zh: "检查模型名称是否正确（INKOS_LLM_MODEL）",
        en: "Check that the model name is correct (INKOS_LLM_MODEL).",
    });
}
export function formatDoctorHintInvalidApiKey(language) {
    return localize(language, {
...
export function formatChapterSyncSummary(language, changed, checked) {
    return localize(language, {
        zh: `已核对 ${checked} 章，修正了 ${changed} 章的 index.json 字数。`,
        en: `Checked ${checked} chapter(s); corrected ${changed} index.json word count(s).`,
    });
}
export function formatChapterSyncMissingFiles(language, numbers) {
    return localize(language, {
...
export function formatChapterDeleteConfirm(language, params) {
    return localize(language, {
...
export function formatChapterDeleteCancelled(language) {
    return localize(language, {
        zh: "已取消。",
        en: "Cancelled.",
    });
}
export function formatChapterDeleteDone(language, params) {
