...
import { formatAutoWriteAlreadyComplete, formatAutoWriteStart, formatNotifyBatchWriteBody, formatNotifyCommandTitle, formatNotifyFailureBody, formatWriteNextComplete, formatWriteNextProgress, formatWriteNextResultLines, resolveCliLanguage, } from "../localization.js";
    ...
    try {
        ...
        if (args.length === 1) {
            ...
            if (isNaN(targetChapter))
                throw new Error(`Expected target chapter number, got "${args[0]}"`);
            bookId = await resolveBookId(undefined, root);
        }
        else if (args.length === 2) {
            targetChapter = parseInt(args[1], 10);
            if (isNaN(targetChapter))
                throw new Error(`Expected target chapter number, got "${args[1]}"`);
        ...
        if (!opts.json)
            log(formatAutoWriteStart(language, bookId, startChapter, targetChapter));
        const wordCount = opts.words ? parseInt(opts.words, 10) : undefined;
        const results = [];
        for (let chapter = startChapter; chapter <= targetChapter; chapter++) {
            if (!opts.json)
                log(formatWriteNextProgress(language, chapter, targetChapter, bookId));
            let result;
            try {
                result = await pipeline.writeNextChapter(bookId, wordCount);
            }
            catch (e) {
                throw new Error(`Chapter ${chapter} failed, stopping auto-write (${results.length} chapter(s) completed this run): ${e instanceof Error ? e.message : String(e)}`, { cause: e });
            }
            results.push(result);
            if (!opts.json) {
            ...
            if (result.status === "state-degraded") {
                throw new Error(`Chapter ${result.chapterNumber} finished in state-degraded status, stopping auto-write. Run "inkos write repair-state ${bookId} ${result.chapterNumber}" first, then re-run inkos auto.`);
            }
        }
        if (opts.json) {
