...
import { formatImportCanonComplete, formatImportCanonStart, formatImportChaptersComplete, formatImportChaptersDiscovery, formatImportChaptersResume, resolveCliLanguage, } from "../localization.js";
...
importCommand
    ...
    try {
        ...
        if (!opts.json)
            log(formatImportCanonStart(language, opts.from, targetBookId));
...
importCommand
    ...
    try {
        ...
        if (existingChapterCount > 0 && !opts.resumeFrom) {
            throw new Error(`Book "${bookId}" already has ${existingChapterCount} chapter(s). ` +
