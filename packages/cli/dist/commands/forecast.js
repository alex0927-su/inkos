...
import { FORECAST_DEFAULT_BRANCHES, FORECAST_DEFAULT_HORIZON, FORECAST_MAX_BRANCHES, FORECAST_MAX_HORIZON, FORECAST_MIN_BRANCHES, FORECAST_MIN_HORIZON, PipelineRunner, createNarrativeForecast, getNarrativeForecast, selectNarrativeBranch, } from "@actalk/inkos-core";
...
forecastCommand
    ...
    try {
        ...
        if (opts.llmBaseUrl)
            config.llm.baseUrl = opts.llmBaseUrl;
        if (opts.model)
        ...
        for (const line of formatBranchLines(result.forecast))
            log(line);
...
export function parseForecastShowArgs(args) {
    if (args.length === 1)
        return { forecastId: args[0] };
    if (args.length === 2)
        return { bookIdArg: args[0], forecastId: args[1] };
    throw new Error("Usage: inkos forecast show [book-id] <forecast-id>");
}
export function parseForecastSelectArgs(args) {
    if (args.length === 2)
        return { forecastId: args[0], branchId: args[1] };
    if (args.length === 3)
        return { bookIdArg: args[0], forecastId: args[1], branchId: args[2] };
    throw new Error("Usage: inkos forecast select [book-id] <forecast-id> <branch-id>");
}
function formatBranchLines(forecast) {
    return forecast.branches.map((branch) => `- ${branch.branchId} "${branch.title}" — intent fit ${branch.intentAlignment.score}/100, ${branch.risks.length} risk(s)`);
}
...
function failForecastCommand(prefix, error, json) {
    if (json) {
