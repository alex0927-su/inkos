...
shortCommand
    ...
    try {
        ...
        const chapterCount = parseBoundedInteger(opts.chapters, SHORT_FICTION_DEFAULT_CHAPTERS, "chapters", SHORT_FICTION_MIN_CHAPTERS, SHORT_FICTION_MAX_CHAPTERS);
        const charsPerChapter = opts.chars === undefined
            ? undefined
            : parseBoundedInteger(opts.chars, language === "en" ? SHORT_FICTION_EN_DEFAULT_WORDS_PER_CHAPTER : SHORT_FICTION_DEFAULT_CHARS_PER_CHAPTER, "chars", language === "en" ? SHORT_FICTION_EN_MIN_WORDS_PER_CHAPTER : SHORT_FICTION_MIN_CHARS_PER_CHAPTER, language === "en" ? SHORT_FICTION_EN_MAX_WORDS_PER_CHAPTER : SHORT_FICTION_MAX_CHARS_PER_CHAPTER);
...
function parseShortFictionLanguage(value) {
    if (value === "zh" || value === "en")
        return value;
    throw new Error("lang must be zh or en.");
}
function resolveShortRunModels(options) {
    return {
...
async function createShortRuntime(root, options) {
    try {
        const config = await loadConfig({ projectRoot: root });
        if (options.llmBaseUrl)
            config.llm.baseUrl = options.llmBaseUrl;
        if (options.model)
        ...
        if (!String(e).includes("inkos.json not found"))
            throw e;
...
function buildEnvLLMConfig(options) {
    ...
    if (!model)
        throw new Error("LLM model is required. Set INKOS_LLM_MODEL or pass --model.");
    return {
        provider: "openai",
...
async function readReference(root, path) {
...
function formatCoverStatus(coverImagePath, coverError) {
    if (coverImagePath)
        return `Cover: ${coverImagePath}`;
    if (coverError)
        return `Cover: skipped (${coverError})`;
    return "Cover: skipped";
}
function logCommandError(prefix, error, json) {
    if (json) {
