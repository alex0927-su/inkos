...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, StateManager } from "@actalk/inkos-core";
import { formatBookBackupCreated, formatBookBackupListEmpty, formatBookCreateCreated, formatBookCreateCreating, formatBookCreateFoundationReady, formatBookCreateLocation, formatBookCreateNextStep, formatBookRestoreDone, resolveCliLanguage, } from "../localization.js";
    ...
    try {
        ...
        if (!opts.force) {
            ...
            const answer = await new Promise((resolve) => {
                rl.question(`Delete "${book.title}" (${bookId})? This will remove ${index.length} chapter(s) and all data. (y/N) `, resolve);
            });
            rl.close();
...
bookCommand
    ...
    .option("--json", "Output JSON")
    .action(async (bookId, backupId, opts) => {
    try {
        const root = findProjectRoot();
        const language = resolveCliLanguage();
