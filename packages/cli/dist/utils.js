...
import { createLLMClient, StateManager, createLogger, createStderrSink, createJsonLineSink, resolveEffectiveLLMConfig, loadLLMEnvLayers, GLOBAL_CONFIG_DIR, GLOBAL_ENV_PATH } from "@actalk/inkos-core";
    ...
    const onStreamProgress = hasLogging
        ? (progress) => {
            if (progress.status === "streaming") {
                logger?.info(`streaming ${Math.round(progress.elapsedMs / 1000)}s, ${progress.totalChars} chars (${progress.chineseChars} CJK)`);
            }
        }
        : undefined;
    return {
...
export async function getLegacyMigrationHint(root, bookId) {
    const state = new StateManager(root);
