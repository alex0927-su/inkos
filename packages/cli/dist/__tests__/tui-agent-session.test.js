...
vi.mock("@actalk/inkos-core", async () => {
    const actual = await vi.importActual("@actalk/inkos-core");
    class PipelineRunnerMock {
        constructor(_config) { }
        async initBook(_book, _options) { }
        async writeNextChapter(_bookId) {
            return {
                chapterNumber: 1,
                title: "雨夜",
                wordCount: 1200,
    ...
    return {
        ...
        PipelineRunner: PipelineRunnerMock,
...
describe("tui agent session bridge", () => {
    ...
    it("runs agent chat and persists raw assistant output into the tui session", async () => {
        ...
        expect(runAgentSessionMock).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("uses explicit slash entries to select book, short, and play surfaces without parsing free text", async () => {
        ...
        expect(runAgentSessionMock).toHaveBeenLastCalledWith(expect.objectContaining({ bookId: null, sessionKind: "book-create", actionSource: "slash" }), "一部海港悬疑长篇", []);
        ...
        expect(runAgentSessionMock).toHaveBeenLastCalledWith(expect.objectContaining({ bookId: null, sessionKind: "short", actionSource: "slash" }), "婚姻背叛后的证据反杀", []);
        await processTuiAgentInput({
            projectRoot,
            input: "/play open 雨夜便利店里时间停止",
            session: createProjectSession(projectRoot),
        });
        expect(runAgentSessionMock).toHaveBeenLastCalledWith(expect.objectContaining({ bookId: null, sessionKind: "play", playMode: "open" }), "雨夜便利店里时间停止", []);
    });
    it("persists a structured proposal and replays its payload and skills only after confirmation", async () => {
        ...
        expect(runAgentSessionMock).toHaveBeenLastCalledWith(expect.objectContaining({
            bookId: null,
        ...
        }), "把上传的故事改成三幕互动影游", expect.any(Array));
...
//# sourceMappingURL=tui-agent-session.test.js.map