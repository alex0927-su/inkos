...
vi.mock("../utils.js", async (importOriginal) => ({
    ...
    log: (message) => logMock(message),
    logError: (message) => logErrorMock(message),
}));
function chapterEntry(number, title, wordCount) {
...
async function setupBook(params) {
    ...
    await writeFile(join(bookDir, "book.json"), JSON.stringify({ id: params.bookId, title: params.bookId, language: "zh" }), "utf-8");
...
describe("inkos chapter sync", () => {
    ...
    it("recounts drifted chapters and prints JSON with --json", async () => {
        ...
        const output = JSON.parse(logMock.mock.calls.at(-1)?.[0]);
        expect(output.changes).toEqual([
            expect.objectContaining({ number: 1, previousWordCount: 3000, wordCount: 9 }),
        ]);
        const savedIndex = JSON.parse(await readFile(join(bookDir, "chapters", "index.json"), "utf-8"));
    ...
    it("prints a bilingual summary when the index is already in sync", async () => {
        ...
        expect(logErrorMock).not.toHaveBeenCalled();
        const printed = logMock.mock.calls.map((call) => call[0]).join("\n");
...
//# sourceMappingURL=chapter-command.test.js.map