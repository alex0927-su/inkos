...
const sourceCliPackageJsonPromise = readFile(resolve(cliDir, "package.json"), "utf-8").then((raw) => JSON.parse(raw));
const sourceStudioPackageJsonPromise = readFile(resolve(studioDir, "package.json"), "utf-8").then((raw) => JSON.parse(raw));
function tarForceLocalArgs() {
...
async function extractPackedPackageJson(packageDir, packDir) {
...
describe.sequential("publish packaging", () => {
    it("rewrites workspace package versions for canary publishing", async () => {
        ...
        try {
            ...
            await writeFile(join(tempRoot, "package.json"), `${JSON.stringify({ name: "inkos", version: "0.4.6" }, null, 2)}\n`);
            await writeFile(join(tempCoreDir, "package.json"), `${JSON.stringify({ name: "@actalk/inkos-core", version: "0.4.6" }, null, 2)}\n`);
            await writeFile(join(tempCliDir, "package.json"), `${JSON.stringify({
                name: "@actalk/inkos",
                version: "0.4.6",
                dependencies: {
            ...
            execFileSync("node", [resolve(workspaceRoot, "scripts/set-package-versions.mjs"), "0.4.8-canary.7", "--root", tempRoot], {
                cwd: workspaceRoot,
                env: process.env,
                encoding: "utf-8",
            });
    ...
    it("verifies publishable manifests before npm publish runs", async () => {
        ...
        expect(cliPackageJson.scripts.prepublishOnly).toBe("node ../../scripts/verify-no-workspace-protocol.mjs .");
        expect(corePackageJson.scripts.prepublishOnly).toBe("node ../../scripts/verify-no-workspace-protocol.mjs .");
    });
    it("accepts canonical workspace protocol in source manifests", async () => {
        ...
        try {
            await mkdir(tempCoreDir, { recursive: true });
            await mkdir(tempCliDir, { recursive: true });
            await writeFile(join(tempRoot, "package.json"), `${JSON.stringify({ name: "inkos", version: "0.5.1" }, null, 2)}\n`);
            ...
            expect(() => execFileSync("node", [resolve(workspaceRoot, "scripts/verify-no-workspace-protocol.mjs"), "packages/core", "packages/cli"], {
                cwd: tempRoot,
                env: process.env,
                encoding: "utf-8",
                stdio: "pipe",
    ...
    it("rejects workspace protocol manifests that normalize to the wrong internal version", async () => {
        ...
        try {
            ...
            expect(() => execFileSync("node", [resolve(workspaceRoot, "scripts/verify-no-workspace-protocol.mjs"), "packages/cli"], {
                cwd: tempRoot,
                env: process.env,
                encoding: "utf-8",
                stdio: "pipe",
