...
describe("studio command", () => {
    ...
    it("launches TypeScript sources through tsx in monorepo mode", async () => {
        ...
        expect(spawnMock).toHaveBeenCalledWith("npx", ["tsx", tsEntry, "/project"], expect.objectContaining({
            cwd: "/project",
            stdio: "inherit",
    ...
    it("launches built JavaScript entries through node", async () => {
        const jsEntry = join("/project", "node_modules", "@actalk", "inkos-studio", "dist", "api", "index.js");
        accessMock.mockImplementation(async (path) => {
            if (path === jsEntry) {
                return;
            }
        ...
        expect(spawnMock).toHaveBeenCalledWith("node", [jsEntry, "/project"], expect.objectContaining({
            cwd: "/project",
            stdio: "inherit",
...
//# sourceMappingURL=studio.test.js.map