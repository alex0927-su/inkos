...
vi.mock("@actalk/inkos-core", () => ({
    ...
    resolveRevisionGate: (book, projectWriting) => book.writing?.revisionGate ?? projectWriting?.revisionGate ?? "strict",
}));
vi.mock("../utils.js", () => ({
    loadConfig: loadConfigMock,
    buildPipelineConfig: buildPipelineConfigMock,
    findProjectRoot: vi.fn(() => "/project"),
    resolveBookId: vi.fn(async (bookId) => bookId ?? "auto-book"),
    log: logMock,
...
describe("inkos revise revision gate", () => {
    ...
    it("passes the book-level revisionGate into the pipeline config", async () => {
        ...
        expect(buildPipelineConfigMock).toHaveBeenCalledWith(expect.anything(), "/project", expect.objectContaining({ revisionGate: "always" }));
    ...
    it("falls back to the project-level revisionGate when the book does not set one", async () => {
        ...
        const { reviseCommand } = await import("../commands/revise.js");
        await reviseCommand.parseAsync(["node", "revise", "demo-book", "1"], { from: "node" });
        expect(buildPipelineConfigMock).toHaveBeenCalledWith(expect.anything(), "/project", expect.objectContaining({ revisionGate: "lenient" }));
    });
    it("defaults to strict when neither book nor project sets a revisionGate", async () => {
        ...
        const { reviseCommand } = await import("../commands/revise.js");
        await reviseCommand.parseAsync(["node", "revise", "demo-book", "1"], { from: "node" });
        expect(buildPipelineConfigMock).toHaveBeenCalledWith(expect.anything(), "/project", expect.objectContaining({ revisionGate: "strict" }));
    });
});
//# sourceMappingURL=revise-gate.test.js.map