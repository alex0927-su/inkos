...
function chapterResult(chapterNumber, status = "ready-for-review") {
    return {
        chapterNumber,
...
describe("inkos auto command", () => {
    ...
    it("writes from the current chapter up to the target chapter with forced auto review", async () => {
        ...
        expect(buildPipelineConfigMock).toHaveBeenCalledWith(expect.anything(), "/project", expect.objectContaining({ chapterReviewMode: "auto" }));
    ...
    it("stops immediately when a chapter write fails", async () => {
        ...
        expect(logErrorMock).toHaveBeenCalledWith(expect.stringContaining("Chapter 2 failed"));
        expect(logErrorMock).toHaveBeenCalledWith(expect.stringContaining("LLM exploded"));
    ...
    it("stops when a chapter ends in state-degraded status", async () => {
        ...
        await autoCommand.parseAsync(["node", "auto", "demo-book", "3"], { from: "node" });
        expect(writeNextChapterMock).toHaveBeenCalledTimes(2);
        expect(logErrorMock).toHaveBeenCalledWith(expect.stringContaining("state-degraded"));
        expect(exitSpy).toHaveBeenCalledWith(1);
    });
});
