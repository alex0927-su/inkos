...
describe("ink dashboard", () => {
    it("renders a codex-like single column with compact status and composer", async () => {
        ...
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "en", projectName: "inkos-demo", activeBookTitle: "Night Harbor Echo", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "", isSubmitting: false }));
    ...
    it("places the initial caret before the placeholder text", async () => {
        const mod = await import("../tui/dashboard.js");
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "en", projectName: "inkos-demo", activeBookTitle: "Night Harbor Echo", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "", isSubmitting: false, showComposerCursor: true }));
        const frame = lastFrame() ?? "";
        expect(frame).toContain("│");
    });
    it("renders the compact status strip directly above the composer", async () => {
        const mod = await import("../tui/dashboard.js");
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "en", projectName: "inkos-demo", activeBookTitle: "Night Harbor Echo", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "continue", isSubmitting: true }));
        const frame = lastFrame() ?? "";
        expect(frame).toContain("writing chapter");
    ...
    it("renders a slash autocomplete dropdown under the composer", async () => {
        const mod = await import("../tui/dashboard.js");
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "en", projectName: "inkos-demo", activeBookTitle: "Night Harbor Echo", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "/c", isSubmitting: false, slashSuggestions: ["/clear", "/config"], selectedSlashIndex: 1 }));
        const frame = lastFrame() ?? "";
    ...
    it("renders a low-frequency bar cursor in the active composer", async () => {
        const mod = await import("../tui/dashboard.js");
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "en", projectName: "inkos-demo", activeBookTitle: "Night Harbor Echo", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "continue", isSubmitting: false, showComposerCursor: true }));
        const frame = lastFrame() ?? "";
        expect(frame).toContain("continue");
        expect(frame).toContain("│");
    });
    it("defaults dashboard chrome to Chinese when locale is zh-CN", async () => {
        const mod = await import("../tui/dashboard.js");
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "zh-CN", projectName: "inkos-demo", activeBookTitle: "\u591C\u6E2F\u56DE\u58F0", modelLabel: "gpt-5.4 (openai)", session: createSession(), inputValue: "", isSubmitting: false }));
        const frame = lastFrame() ?? "";
    ...
    it("surfaces the shared creation draft when no active book exists yet", async () => {
        ...
        const { lastFrame } = render(_jsx(mod.InkTuiDashboard, { locale: "zh-CN", projectName: "inkos-demo", modelLabel: "gpt-5.4 (openai)", session: draftSession, inputValue: "", isSubmitting: false }));
        const frame = lastFrame() ?? "";
        expect(frame).toContain("草稿 夜港账本");
    });
});
//# sourceMappingURL=tui-dashboard.test.js.map