...
function buildTestEnv(overrides) {
    const baseEnv = Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith("INKOS_")
...
function runStderr(args, options) {
    try {
...
describe("CLI integration", () => {
    ...
    describe("inkos interact", () => {
        it("returns the agent-session JSON contract for natural-language interactions", async () => {
            ...
            try {
                await writeFile(envPath, Object.entries(failingLlmEnv).map(([key, value]) => `${key}=${value}`).join("\n"), "utf-8");
    ...
    describe("inkos review", () => {
        it("preserves the original chapter snapshot when approving review", async () => {
            ...
            await expect(readFile(join(storyDir, "snapshots", "1", "current_state.md"), "utf-8")).resolves.toBe("State at ch1");
            await expect(readFile(join(storyDir, "snapshots", "1", "pending_hooks.md"), "utf-8")).resolves.toBe("Hooks at ch1");
    ...
    describe("inkos export", () => {
        beforeAll(async () => {
            ...
            await writeFile(join(bookDir, "chapters", "0001_Dawn_Ledger.md"), "# 第1章 Dawn Ledger\n\n正文。\n", "utf-8");
        });
