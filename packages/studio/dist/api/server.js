...
import { StateManager, PipelineRunner, createLLMClient, createLogger, createInteractionToolsFromDeps, computeAnalytics, loadProjectConfig, loadProjectSession, processProjectInteractionRequest, resolveSessionActiveBook, listBookSessions, loadBookSession, appendManualSessionMessages, createAndPersistBookSession, renameBookSession, deleteBookSession, migrateBookSession, SessionAlreadyMigratedError, abortAgentSession, runAgentSession, resolveServicePreset, resolveServiceProviderFamily, resolveServiceModelsBaseUrl, guessServiceFromBaseUrl, resolveServiceModel, loadSecrets, saveSecrets, listModelsForService, isApiKeyOptionalForEndpoint, getAllEndpoints, probeModelsFromUpstream, fetchWithProxy, chatCompletion, runWorkerAgent, buildExportArtifact, evaluateBookQuality, ConsolidatorAgent, DetectionConfigSchema, ResearchSearchConfigSchema, GLOBAL_ENV_PATH, COVER_PROVIDER_PRESETS, createPlayDB, PlayStore, buildPlayEntityImagePrompt, buildPlaySceneImagePrompt, generatePlayImage, readPlayImageManifest, readPlayImageSettings, writePlayImageSettings, Scheduler, coverSecretKey, normalizeCoverBaseUrl, resolveCoverProviderPreset, SessionKindSchema, normalizeActionSource as normalizeCoreActionSource, normalizeActionPayload as normalizeCoreActionPayload, normalizePlayMode as normalizeCorePlayMode, normalizeRequestedIntent as normalizeCoreRequestedIntent, normalizeSkillIdList as normalizeCoreSkillIdList, inferLanguage, ingestMaterial, createSkillRegistry, loadAvailableAgentSkills, activatedSkillIds, mergeActivatedSkillGuidance, resolveProductionSkillActivations, parseAgentSkillDocument, getBuiltinPrompt, listBuiltinPromptPacks, listBuiltinPrompts, loadPromptPackPrompt, promptOverridePath, toPosixPath, createGenerateCoverTool, createInteractiveFilmCreationTool, createPlayStartTool, createScriptCreationTool, createShortFictionRunTool, createStoryboardCreationTool, createTranslationCreateTool, createFanficBookTool, createContinuationImportTool, createSpinoffBookTool, createImitationBookTool, createSubAgentTool, createDraftStructureTool, createConnectChoiceTool, createRemoveNodeTool, createLLMTranslationModel, deleteLatestChapter, executeEditTransaction, listChapterVersions, readChapterPlanDocument, readChapterUserBrief, readChapterVersion, saveChapterUserBrief, createTranslationProjectFromFile, loadTranslationChapter, loadTranslationManifest, runTranslationProject, writeTranslationExport, filmLLMDepsFromClient, applyGraphDelta, loadStoryGraph, reviewStoryGraph, exportInk, buildPlayableHtml, analyzeEmotionalArcs, analyzePathDistribution, generateNodeImage, defaultNodeImageDeps, } from "@actalk/inkos-core";
...
import { deleteStudioTaskSnapshot, loadStudioTaskSnapshot, saveStudioTaskSnapshot, } from "./task-store.js";
function normalizeStudioLanguage(value) {
...
function pipelineStages(agent, lang = "zh") {
...
function resolveToolLabel(tool, agent, lang = "zh") {
...
function formatTaskElapsed(ms, lang) {
    const totalSeconds = Math.max(0, Math.floor(ms / 1000));
    const minutes = Math.floor(totalSeconds / 60);
    const seconds = totalSeconds % 60;
    if (minutes === 0)
        return pick(lang, `${seconds} 秒`, `${seconds}s`);
 ...
 */
function buildRunningTaskContextBlock(task, lang) {
...
async function buildTarArchive(sourceDir, packageRootName) {
...
async function listArchiveFiles(dir, prefix = "") {
...
function nonTextModelMessage(modelId, lang = "zh") {
    return pick(lang, `模型 ${modelId} 不适合文本聊天/写作。请在模型选择器中改用文本模型，例如 gemini-2.5-flash、gemini-2.5-pro 或对应服务的 chat 模型。`, `Model ${modelId} is not suitable for text chat/writing. Pick a text model in the model selector, e.g. gemini-2.5-flash, gemini-2.5-pro, or the service's chat model.`);
}
...
function normalizeProjectGeneratedPath(root, rawPath, code) {
    let relPath;
    try {
        relPath = decodeURIComponent(rawPath).replace(/^\/+/u, "");
    }
...
function resolveProjectTextArtifactFile(root, rawPath) {
...
function normalizeStudioSkillIdList(value, field) {
    try {
...
function normalizeStudioSkillId(value, field = "skillId") {
    const [id] = normalizeStudioSkillIdList([value], field);
    if (!id)
        throw new ApiError(400, "INVALID_SKILL_ID", `Invalid ${field}: empty`);
    return id;
}
...
async function normalizeAgentAttachments(root, sessionId, value) {
    if (value === undefined || value === null)
        return [];
...
async function storeTranslationUpload(root, payload) {
...
async function storeProjectUpload(root, payload, options) {
...
function projectSkillsDir(root) {
    return join(root, ".agents", "skills");
}
function projectSkillDir(root, id) {
    return join(projectSkillsDir(root), id);
}
function projectSkillPath(root, id) {
    return join(projectSkillDir(root, id), "SKILL.md");
}
function toStudioSkill(skill, root, projectSkillIds) {
...
function normalizeSkillImportFiles(value) {
...
async function importStudioSkillFolder(root, payload) {
    ...
    try {
        ...
        throw new ApiError(400, "INVALID_SKILL_MANIFEST", error instanceof Error ? error.message : String(error));
    }
...
async function loadStudioSkills(root) {
...
async function toStudioPromptPackPrompt(root, prompt) {
...
function normalizeStudioPromptId(value) {
...
async function listProjectSkillIds(root) {
    try {
...
function validateAgentActionExecution(args) {
    ...
    if (failedExec) {
        ...
        return pick(lang, `${failedExec.label} 执行失败：${detail}`, `${failedExec.label} failed: ${detail}`);
    }
    if (args.agentBookId
        && args.requestedIntent === "write_next"
        && !hasSuccessfulSubAgentExec(args.collectedToolExecs, "writer")) {
        return pick(lang, "模型声称已完成下一章，但没有实际调用写作工具。请重试；如果仍失败，请检查模型是否支持工具调用。", "The model claimed the next chapter is done, but it never called the writing tool. Retry; if it keeps failing, check whether the model supports tool calls.");
    }
    if (!args.agentBookId
        && args.requestedIntent === "create_book"
        && !hasSuccessfulSubAgentExec(args.collectedToolExecs, "architect")) {
        return pick(lang, "已确认建书，但模型没有实际调用建书工具。请重试；如果仍失败，请检查模型是否支持工具调用。", "Book creation was confirmed, but the model never called the book setup tool. Retry; if it keeps failing, check whether the model supports tool calls.");
    }
    if (args.requestedIntent === "short_run" && !hasSuccessfulToolExec(args.collectedToolExecs, "short_fiction_run")) {
        return pick(lang, "已确认生成短篇，但模型没有实际调用短篇生产工具。请重试；如果仍失败，请检查模型是否支持工具调用。", "Short fiction was confirmed, but the model never called the short fiction tool. Retry; if it keeps failing, check whether the model supports tool calls.");
    }
    if (args.requestedIntent === "play_start" && !hasSuccessfulToolExec(args.collectedToolExecs, "play_start")) {
        return pick(lang, "已确认启动互动世界，但模型没有实际调用互动世界工具。请重试；如果仍失败，请检查模型是否支持工具调用。", "Starting the interactive world was confirmed, but the model never called the interactive world tool. Retry; if it keeps failing, check whether the model supports tool calls.");
    }
    if (args.requestedIntent === "generate_cover" && !hasSuccessfulToolExec(args.collectedToolExecs, "generate_cover")) {
        return pick(lang, "已确认生成封面，但模型没有实际调用封面工具。请重试；如果仍失败，请检查模型是否支持工具调用。", "Cover generation was confirmed, but the model never called the cover tool. Retry; if it keeps failing, check whether the model supports tool calls.");
    }
    return undefined;
}
...
function formatAgentFailure(message, lang = "zh") {
    ...
    if (kind === "internal") {
        return {
            ...
            message: pick(lang, `云溪 内部流程错误：${message}`, `云溪 internal pipeline error: ${message}`),
            status: 500,
        };
    }
    return { code: "AGENT_ERROR", message, status: 500 };
}
function formatAgentActionFailure(message, lang) {
...
function hasSuccessfulToolOwnedResponse(execs) {
...
function toolResultText(result, lang = "zh") {
...
async function executeConfirmedProductionAction(args) {
    ...
    const productionSkills = (capability) => mergeActivatedSkillGuidance(resolveProductionSkillActivations(skillResolution.availableSkills, capability), requestedSkillActivations);
    let tool;
    let params;
    let agent;
    if (args.requestedIntent === "create_book") {
        ...
        if (!direction)
            throw new ApiError(400, "CONFIRMED_ACTION_PAYLOAD_INCOMPLETE", pick(lang, "确认短篇缺少方向，请重新生成确认卡。", "The short fiction confirmation is missing a direction. Regenerate the confirmation card."));
        ...
        if (!projectId)
            throw new ApiError(400, "INVALID_ID", "interactive-film action requires a project id (bookId)");
    ...
    const exec = {
        ...
        status: "running",
        args: params,
        stages: agent ? pipelineStages(agent, lang)?.map(label => ({ label, status: "pending" })) : undefined,
        startedAt: Date.now(),
    };
    ...
    try {
        const result = await tool.execute(id, params, args.signal, (partialResult) => {
        ...
        const resultIsError = Boolean(result?.isError);
...
function normalizeServiceModelIds(value) {
    if (!Array.isArray(value))
        return [];
    const seen = new Set();
    const models = [];
    for (const item of value) {
        ...
        if (!model || seen.has(key) || !isTextChatModelId(model))
            continue;
        seen.add(key);
        models.push(model);
    }
...
function mergeServiceModelIds(...groups) {
...
function normalizeChapterReviewMode(mode) {
    return mode === "manual" ? "manual" : "auto";
}
function readProjectChapterReviewMode(config) {
...
function readBookChapterReviewMode(rawBook) {
    const writing = rawBook.writing && typeof rawBook.writing === "object" && !Array.isArray(rawBook.writing)
        ? rawBook.writing
        : undefined;
    if (!writing || writing.reviewMode !== "manual" && writing.reviewMode !== "auto")
        return undefined;
    return writing.reviewMode;
}
async function loadRawBookConfig(root, bookId) {
    const raw = await readFile(join(root, "books", bookId, "book.json"), "utf-8");
    return JSON.parse(raw);
}
async function resolveBookChapterReviewMode(root, bookId, projectMode) {
...
function normalizeRevisionGate(gate) {
    return gate === "lenient" || gate === "always" ? gate : "strict";
}
function readProjectRevisionGate(config) {
    const writing = config.writing && typeof config.writing === "object" && !Array.isArray(config.writing)
        ? config.writing
        : {};
    return normalizeRevisionGate(writing.revisionGate);
}
function readBookRevisionGate(rawBook) {
    const writing = rawBook.writing && typeof rawBook.writing === "object" && !Array.isArray(rawBook.writing)
        ? rawBook.writing
        : undefined;
    if (!writing || writing.revisionGate !== "strict" && writing.revisionGate !== "lenient" && writing.revisionGate !== "always")
        return undefined;
    return writing.revisionGate;
}
async function resolveBookRevisionGate(root, bookId, projectGate) {
    if (!bookId || !isSafeBookId(bookId))
        return projectGate;
    try {
        const rawBook = await loadRawBookConfig(root, bookId);
...
async function withTimeout(promise, timeoutMs, label, lang = "zh") {
    let timeout;
    try {
        return await Promise.race([
            promise,
            new Promise((_, reject) => {
                timeout = setTimeout(() => reject(new Error(pick(lang, `${label} 超时（${timeoutMs}ms）`, `${label} timed out (${timeoutMs}ms)`))), timeoutMs);
            }),
        ]);
    }
    finally {
...
function formatServiceProbeError(args) {
    ...
    const upstreamPrefix = (detail) => pick(lang, `\n上游返回：${detail}`, `\nUpstream response: ${detail}`);
    if (args.service === "google") {
        return [
            ...
            pick(lang, "1. API Key 是否来自 Google AI Studio 的 Gemini API key，而不是 OAuth、Vertex AI 或其它 Google 服务凭据。", "1. The API Key is a Gemini API key from Google AI Studio, not an OAuth, Vertex AI, or other Google service credential."),
            pick(lang, "2. 该 key 所属项目是否已启用 Gemini API，并且没有被限制到其它 API、来源或服务。", "2. The key's project has the Gemini API enabled and is not restricted to other APIs, origins, or services."),
            pick(lang, "3. 当前地区/账号是否允许访问 Gemini API。", "3. Your region/account is allowed to access the Gemini API."),
            pick(lang, "4. 如果 key 曾经泄露，请在 AI Studio 重新生成后再保存。", "4. If the key was ever leaked, regenerate it in AI Studio before saving."),
    ...
    if (args.service === "moonshot" || args.service === "kimiCodingPlan" || args.service === "kimicode") {
        return [
            ...
            context,
            "",
            pick(lang, "请优先检查模型是否可用，以及 kimi-k2.x 这类模型是否需要 temperature=1。", "Check first whether the model is available, and whether models like kimi-k2.x require temperature=1."),
    ...
    return [
        ...
        context,
        "",
        pick(lang, "请检查 API Key、模型可用性、账号额度，以及协议类型是否匹配该服务商。", "Check the API Key, model availability, account quota, and whether the protocol type matches this service."),
        rawDetail ? upstreamPrefix(rawDetail) : "",
    ].filter(Boolean).join("\n");
}
async function fetchModelsFromServiceBaseUrl(serviceId, baseUrl, apiKey, proxyUrl, lang = "zh") {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return {
                models: [],
                error: pick(lang, `服务商返回 ${res.status}: ${body.slice(0, 200)}`, `Service returned ${res.status}: ${body.slice(0, 200)}`),
...
function buildBearerAuthHeaders(apiKey, lang = "zh") {
    const trimmed = apiKey?.trim() ?? "";
    if (!trimmed)
        return {};
    if (!/^[\x20-\x7e]+$/.test(trimmed)) {
        throw new Error(pick(lang, "API Key 只能包含英文、数字和常见 ASCII 符号，请检查是否误粘贴了中文说明。", "API Key may only contain ASCII letters, digits, and common symbols. Check whether you pasted explanatory text by mistake."));
    }
...
async function probeServiceCapabilities(args) {
    ...
    if (modelsResponse.authFailed) {
        return {
            ok: false,
            models: [],
            error: modelsResponse.error ?? pick(lang, "API Key 无效或无权访问模型列表。", "API Key is invalid or has no access to the model list."),
        };
    }
    ...
    if (discoveredModels.length > 0) {
        if (!discoveredFirstModel || !isTextChatModelId(discoveredFirstModel)) {
            return {
                ...
                error: pick(lang, "模型列表可访问，但没有发现可用于文本对话的模型。", "The model list is reachable, but no model usable for text chat was found."),
            };
        }
        return {
            ok: true,
    ...
    if (modelCandidates.length === 0) {
        return {
            ok: false,
            models: [],
            error: pick(lang, "无法自动确定模型，请先填写可用模型或提供支持 /models 的服务端点。", "Could not determine a model automatically. Fill in an available model first, or provide a service endpoint that supports /models."),
        };
    }
    ...
    for (const model of modelCandidates) {
        for (const plan of buildProbePlans(args.preferredApiFormat, args.preferredStream)) {
            ...
            try {
                ...
                chatCompletion(client, model, [{ role: "user", content: "Reply with OK only." }], { maxTokens: 16, retry: false }), SERVICE_CHAT_PROBE_TIMEOUT_MS, "service connection test", lang);
...
// --- Server factory ---
export function createStudioServer(initialConfig, root, overrides = {}) {
    const app = new Hono();
    const state = new StateManager(root);
    let cachedConfig = initialConfig;
    const activeConfirmedTasks = new Map();
    ...
    // controller 找到刚启动的任务。
    const reservedProductionSessions = new Map();
    ...
    // 之后）。所有手动追加统一走这个守卫。
    const appendSessionMessagesUnlessDeleted = async (projectRoot, sessionId, messages, input, options) => {
        if (deletedSessionIds.has(sessionId))
            return;
        await appendManualSessionMessages(projectRoot, sessionId, messages, input, options);
    };
    const persistConfirmedTask = async (sessionId, requestedIntent, exec, sourceRequestId) => {
        if (deletedSessionIds.has(sessionId))
            return;
        const snapshot = {
            version: 1,
    ...
    const loadReconciledTaskSnapshot = async (sessionId) => {
        ...
        if (!running || activeConfirmedTasks.has(task.execution.id))
            return task;
        ...
        const reconciled = {
            ...
            execution: {
                ...task.execution,
                status: "error",
                error: pick(lang, "任务已中断：Studio 服务在任务运行期间重启，任务未能继续。请重新发起。", "Task interrupted: the Studio server restarted while this task was running. Please start it again."),
                completedAt,
            },
        };
    ...
    const findActiveRunningTask = async (sessionId) => {
    ...
    const findRunningTaskController = async (sessionId) => {
    ...
    async function currentProjectLanguage() {
    ...
    async function buildPipelineConfig(overrides) {
        ...
        const projectReviewMode = readProjectChapterReviewMode(currentConfig);
        const chapterReviewMode = await resolveBookChapterReviewMode(root, overrides?.bookIdForSettings, projectReviewMode);
        const projectRevisionGate = readProjectRevisionGate(currentConfig);
    ...
    app.post("/api/v1/books/:id/chapters/:num/workspace/inspiration", async (c) => {
        ...
        try {
            ...
            const response = await runWorkerAgent(pipelineConfig.client, pipelineConfig.model, [
                {
                    role: "system",
                    content: language === "en"
                        ? [
    ...
    app.get("/api/v1/books/:id/chapters/:num/versions/:versionId", async (c) => {
        ...
        const num = parseInt(c.req.param("num"), 10);
        try {
            const content = await readChapterVersion(state.bookDir(id), num, c.req.param("versionId"));
    ...
    app.post("/api/v1/books/:id/chapters/:num/versions/:versionId/restore", async (c) => {
        ...
        try {
            const fullText = await readChapterVersion(state.bookDir(id), num, c.req.param("versionId"));
    ...
    app.post("/api/v1/books/:id/write-next", async (c) => {
        ...
        pipeline.writeNextChapter(id, body.wordCount).then((result) => {
    ...
    app.post("/api/v1/books/:id/chapters/:num/approve", async (c) => {
        ...
        try {
            ...
            const updated = index.map((ch) => ch.number === num ? { ...ch, status: "approved" } : ch);
    ...
    app.get("/api/v1/events", (c) => {
        return streamSSE(c, async (stream) => {
            ...
            if (sessionId) {
                const task = await loadReconciledTaskSnapshot(sessionId);
                if (task)
                    await stream.writeSSE({ event: "task:snapshot", data: JSON.stringify(task) });
            }
            // Keep alive
    ...
    app.get("/api/v1/services", async (c) => {
        ...
        try {
            const config = await loadRawConfig(root);
            configuredServices = normalizeServiceConfig(config.llm?.services);
        }
        catch { /* no config file */ }
        const configuredBankServices = new Set(configuredServices
    ...
    app.post("/api/v1/services/config/import-env", async (c) => {
        ...
        if (!env || !env.values.apiKey) {
            return c.json({
                error: pick(await currentProjectLanguage(), "未检测到可导入的 LLM 环境变量配置，或缺少 INKOS_LLM_API_KEY。", "No importable LLM environment variable configuration was detected, or INKOS_LLM_API_KEY is missing."),
            }, 400);
        }
        const config = await loadRawConfig(root);
    ...
    app.put("/api/v1/services/config", async (c) => {
        ...
        if (body.configSource === "env") {
            return c.json({
                error: pick(await currentProjectLanguage(), "Studio 运行时不支持切换到 env；env 只在 CLI/daemon/部署运行时作为覆盖层使用。", "The Studio runtime does not support switching to env; env only acts as an override layer in the CLI/daemon/deployment runtimes."),
            }, 400);
        }
    ...
    app.put("/api/v1/cover/config", async (c) => {
        ...
        if (requestedBaseUrl && !baseUrl) {
            return c.json({
                error: pick(await currentProjectLanguage(), "封面 Base URL 必须是有效的 HTTP(S) 地址，且不能包含账号、查询参数或锚点。", "Cover Base URL must be a valid HTTP(S) URL without credentials, query parameters, or fragments."),
            }, 400);
        }
        const config = await loadRawConfig(root);
        config.llm = config.llm ?? {};
    ...
    app.put("/api/v1/cover/secret/:service", async (c) => {
        ...
        if (trimmedKey && !isHeaderSafeApiKey(trimmedKey)) {
            return c.json({
                error: pick(await currentProjectLanguage(), "API Key 包含不能放入 HTTP Authorization header 的字符，请只粘贴原始密钥。", "API Key contains characters that cannot go into an HTTP Authorization header. Paste only the raw key."),
            }, 400);
        }
        const secrets = await loadSecrets(root);
    ...
    app.put("/api/v1/services/:service/secret", async (c) => {
        ...
        if (trimmedKey) {
            if (!isHeaderSafeApiKey(trimmedKey)) {
                return c.json({
                    ok: false,
                    error: pick(await currentProjectLanguage(), "API Key 只能包含可放进 HTTP Authorization header 的非空白 ASCII 字符；请不要粘贴连接失败提示或诊断文本。", "API Key may only contain non-whitespace ASCII characters that fit in an HTTP Authorization header; do not paste connection failure hints or diagnostic text."),
                }, 400);
            }
    ...
    app.get("/api/v1/services/models", async (c) => {
        ...
        const configuredServices = normalizeServiceConfig(config.llm?.services);
    ...
    app.get("/api/v1/prompt-packs", async (c) => {
        const prompts = await Promise.all(listBuiltinPrompts().map((prompt) => toStudioPromptPackPrompt(root, prompt)));
        return c.json({
    ...
    app.post("/api/v1/agent", async (c) => {
        const { instruction, activeBookId, sessionId: reqSessionId, clientRequestId: reqClientRequestId, sessionKind: reqSessionKind, actionSource: reqActionSource, requestedIntent: reqRequestedIntent, actionPayload: reqActionPayload, requestedSkills: reqRequestedSkills, disabledSkills: reqDisabledSkills, attachments: reqAttachments, playMode: reqPlayMode, model: reqModel, service: reqService, } = await c.req.json();
        ...
        try {
            ...
            const refreshBookSessionFromTranscript = async () => {
                ...
                if (deletedSessionIds.has(bookSession.sessionId))
                    return;
                ...
                try {
                    ...
                    if (/API key/i.test(msg)) {
                        return c.json({
                            ...
                            response: pick(language, `请先在模型配置中为 ${reqService} 填写 API Key，然后再试。`, `Fill in an API Key for ${reqService} in the model settings, then try again.`),
                        }, 400);
                    }
                    throw e;
                }
            ...
            if (confirmedIntent && confirmedTaskId) {
                const productionTaskBusyResponse = () => {
                    const message = pick(surfaceLanguage, "当前会话已有一个生产任务在运行，请等它完成，或先用停止按钮结束它，再发起新任务。", "A production task is already running in this session. Wait for it to finish, or stop it first, then start a new task.");
                    return c.json({
                ...
                try {
                    ...
                    const exec = await executeConfirmedProductionAction({
                        ...
                        onTaskChange: (taskExec) => persistConfirmedTask(bookSession.sessionId, confirmedIntent, taskExec, sourceRequestId),
                        ...(playMode ? { playMode } : {}),
                    });
                    ...
                    if (event.type === "tool_execution_end") {
                        ...
                        if (exec) {
                            ...
                            if (event.isError)
                                ...
                                exec.result = summarizeToolResult(event.result);
            ...
            if (!result.responseText) {
                ...
                const emptyMessage = pick(language, "模型未返回文本内容。请检查协议类型（chat/responses）、流式开关或上游服务兼容性。", "The model returned no text content. Check the protocol type (chat/responses), the streaming switch, or upstream service compatibility.");
                if (resolveCreatedBookIdFromToolExecs(collectedToolExecs)) {
                    await finalizeCreatedBook();
                }
                return c.json({
            ...
            if (/already processing|prompt.*queue/i.test(msg)) {
                return c.json({
                    ...
                    response: pick(language, "正在处理中，请等待当前操作完成后再发送。", "Still processing. Wait for the current operation to finish before sending again."),
                }, 429);
            }
    ...
    app.put("/api/v1/books/:id/chapter-review-mode", async (c) => {
        ...
        try {
            ...
            if (mode === "inherit") {
                ...
                rawBook.writing = {
                    ...(rawBook.writing && typeof rawBook.writing === "object" && !Array.isArray(rawBook.writing) ? rawBook.writing : {}),
    ...
    app.get("/api/v1/interactive-films", async (c) => {
        ...
        for (const projectId of entries) {
            ...
            try {
                ...
                if (graph)
                    films.push({ projectId, title: graph.title || projectId });
            }
    ...
    app.post("/api/v1/translations/:id/run", async (c) => {
        ...
        try {
            ...
            throw new ApiError(isUpstream ? 502 : 500, "TRANSLATION_RUN_FAILED", message || "Translation run failed.");
        }
    });
    ...
    app.post("/api/v1/projects/:id/story-graph/delta", async (c) => {
        ...
        const { graph, rev } = await applyGraphDelta({ projectRoot: root, projectId: id, delta: delta });
...
export async function startStudioServer(root, port = 4567, options) {
    ...
    if (options?.staticDir) {
        ...
        app.get("/*", async (c, next) => {
            ...
            if (reqPath === "/" || reqPath.indexOf("/", 1) !== -1)
                return await next();
    ...
    console.log(`云溪 Studio running on http://localhost:${port}`);
