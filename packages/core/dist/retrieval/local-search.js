...
export class LocalSearchIndex {
    ...
    replaceScope(scope, documents) {
        ...
        const existing = this.db.prepare("SELECT document_id AS id, content_hash AS contentHash FROM retrieval_documents WHERE scope = ?").all(scope);
    ...
    `);
        const remove = this.db.prepare("DELETE FROM retrieval_documents WHERE scope = ? AND document_id = ?");
        this.db.exec("BEGIN IMMEDIATE");
        try {
            for (const document of normalized) {
                if (existingHashes.get(document.id) === document.contentHash)
                    continue;
                upsert.run(document.id, document.scope, document.kind, document.source, document.title, document.body, document.titleTokens, document.bodyTokens, document.metadataJson, document.contentHash);
            }
            for (const row of existing) {
                if (!keepIds.has(row.id))
                    remove.run(scope, row.id);
            }
            this.db.exec("COMMIT");
        }
        catch (error) {
    ...
    search(query, options) {
      ...
      LIMIT ?
    `).all(match, options.scope, ...kinds, limit);
        return rows.map((row) => ({
            id: row.id,
            scope: row.scope,
            kind: row.kind,
...
export function splitMarkdownForSearch(markdown) {
    const segments = [];
    let heading = "";
    const blockPattern = /(?:^|\n)([^\n](?:[\s\S]*?))(?=\n\s*\n|$)/g;
...
export function tokenizeSearchText(text) {
    ...
    for (const part of segmenter.segment(normalized)) {
        ...
        if (!part.isWordLike || !token)
            continue;
        if (/^[\p{Script=Latin}\p{N}_-]+$/u.test(token) && token.length < 2)
            continue;
        tokens.push(token);
    }
...
function normalizeDocument(document, scope) {
