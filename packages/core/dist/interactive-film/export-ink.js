    ...
    for (const node of graph.nodes) {
        ...
        for (const c of node.choices) {
            ...
            for (const e of c.effects ?? [])
                lines.push(effectLine(e));
            lines.push(`    -> ${knot(c.targetNodeId)}`);
        }
    }
    return lines.join("\n") + "\n";
}
//# sourceMappingURL=export-ink.js.map