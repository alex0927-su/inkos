...
export function emotionScore(word) {
    const w = word.trim();
    if (!w)
        return 0;
    if (w in EMOTION_LEXICON)
        return EMOTION_LEXICON[w];
...
export function nodeEmotion(node) {
...
export function analyzeEmotionalArcs(graph) {
...
export function analyzePathDistribution(graph) {
    const { paths, truncated } = enumerateRuntimePaths(graph);
    const byEnding = {};
