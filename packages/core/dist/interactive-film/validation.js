...
export function validateStoryGraph(graph) {
    const issues = [];
...
export function reviewStoryGraph(graph) {
    const issues = [...validateStoryGraph(graph).issues];
    const reads = new Set();
    ...
    for (const n of graph.nodes)
        for (const c of n.choices)
            incoming.add(c.targetNodeId);
    for (const node of graph.nodes) {
        if (node.type !== "start" && node.type !== "ending" && !incoming.has(node.id)) {
            if (issues.some((i) => i.code === "UNREACHABLE" && i.nodeIds.includes(node.id)))
                continue;
            issues.push({
                code: "ISOLATED_NODE",
                level: "info",
...
function computeEdgeReachable(graph) {
...
function findLongLinearChainHeads(graph, threshold) {
    const nodeById = new Map(graph.nodes.map((n) => [n.id, n]));
    const isChainNode = (id) => {
        const n = nodeById.get(id);
        return n !== undefined && n.type === "normal" && n.choices.length === 1;
    };
    const incomingIsChainNode = new Map();
    for (const n of graph.nodes) {
        for (const c of n.choices) {
            if (isChainNode(n.id)) {
    ...
    const heads = [];
    for (const node of graph.nodes) {
        if (!isChainNode(node.id))
            continue;
        if (incomingIsChainNode.get(node.id))
            continue;
        let length = 0;
        let cur = node.id;
        ...
        while (cur !== undefined && isChainNode(cur) && !visited.has(cur)) {
            visited.add(cur);
            length++;
            const chainNode = nodeById.get(cur);
