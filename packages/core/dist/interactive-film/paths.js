...
function varStateKey(vars) {
    return Object.keys(vars)
        .sort()
        .map((key) => `${key}:${JSON.stringify(vars[key])}`)
        .join("|");
}
export function enumerateRuntimePaths(graph, opts) {
    ...
    const walk = (nodeId, vars, trail, onPath, depth) => {
        ...
        if (onPath.has(visitKey))
            return; // No-op cycles are already represented by this exact node+state.
