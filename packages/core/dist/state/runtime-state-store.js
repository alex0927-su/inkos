...
import { bootstrapStructuredStateFromMarkdown, parseChapterSummariesMarkdown, parseCurrentStateFacts, parsePendingHooksMarkdown, } from "./state-bootstrap.js";
...
export async function loadRuntimeStateSnapshotAtChapter(params) {
    const snapshotDir = join(params.bookDir, "story", "snapshots", String(params.chapterNumber));
    ...
    if (manifest && currentState && hooks && chapterSummaries) {
        return validateLoadedSnapshot({ manifest, currentState, hooks, chapterSummaries }, `runtime snapshot at chapter ${params.chapterNumber}`);
    }
    ...
    return validateLoadedSnapshot(markdownSnapshot, `markdown runtime snapshot at chapter ${params.chapterNumber}`);
}
...
function validateLoadedSnapshot(snapshot, label) {
