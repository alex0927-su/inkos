 ...
 */
export function isBookRulesShim(raw) {
    return (/本书规则（兼容指针——已废弃）/.test(raw)
 ...
 */
export function tryParseBookRulesFrontmatter(raw, onError) {
    const stripped = raw.replace(/^```(?:md|markdown|yaml)?\s*\n/, "").replace(/\n```\s*$/, "");
    const fmMatch = stripped.match(/---\s*\n([\s\S]*?)\n---\s*\n?([\s\S]*)$/);
    if (!fmMatch)
        return null;
...
function parseMarkdownBookRules(raw) {
    const protagonistSection = extractMarkdownSection(raw, ["主角", "Protagonist"]);
    const protagonistName = readLabeledValue(protagonistSection, ["名字", "姓名", "name", "protagonist"])
...
function extractMarkdownSection(raw, headings) {
...
function readLabeledValue(raw, labels) {
    if (!raw.trim())
        return undefined;
...
function detectNarrativePerson(raw) {
    if (/第一人称|first[-\s]?person|\bfirst\b/i.test(raw))
        return "first";
    if (/第三人称|third[-\s]?person|\bthird\b/i.test(raw))
        return "third";
    return undefined;
}
function normalizeFanficMode(value) {
    if (!value)
        return undefined;
    const normalized = value.trim().toLowerCase();
    if (normalized === "canon" || /正典|原作空白|原作视角/.test(value))
        return "canon";
    if (normalized === "au" || /平行|分歧|if线/i.test(value))
        return "au";
    if (normalized === "ooc" || /性格偏离/i.test(value))
        return "ooc";
    if (normalized === "cp" || /配对|感情线/i.test(value))
        return "cp";
    return undefined;
}
