...
function lenientArray(item) {
    return z.preprocess((v) => (Array.isArray(v) ? v : []), z.array(z.unknown()).transform((arr) => arr.flatMap((entry) => {
...
function edgeIdFromParts(fromId, type, toId, index) {
...
function backfillUpsertIds(container, prefix, labelKey) {
    if (!container || typeof container !== "object" || Array.isArray(container))
        return container;
    const c = container;
    if (!Array.isArray(c.upsert))
        return c;
...
function buildLabelToId(entities) {
    const map = new Map();
    const c = entities;
    if (!Array.isArray(c?.upsert))
        return map;
...
function isLowInformationEdgeId(id, type) {
    if (typeof id !== "string")
        return true;
...
function backfillEdges(container, eventId, labelToId) {
    if (!container || typeof container !== "object" || Array.isArray(container))
        return container;
    const c = container;
    if (!Array.isArray(c.upsert))
    ...
    const resolve = (v) => (hasText(v) && labelToId.has(v.trim()) ? labelToId.get(v.trim()) : v);
    c.upsert = c.upsert.map((item, i) => {
        if (!item || typeof item !== "object" || Array.isArray(item))
            return item;
        const o = { ...item };
        for (const [alias, canonical] of EDGE_KEY_ALIASES) {
            if (!hasText(o[canonical]) && hasText(o[alias]))
                o[canonical] = o[alias];
        }
        // Endpoints given by entity label (or id) resolve to the entity's id.
        o.fromId = resolve(o.fromId);
        o.toId = resolve(o.toId);
        if (!hasText(o.validFromEventId))
            o.validFromEventId = eventId;
        if (!hasText(o.sourceEventId))
            o.sourceEventId = eventId;
...
function normalizePlayMutation(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return value;
    const v = { ...value };
    if (Array.isArray(v.entities))
        v.entities = { upsert: v.entities };
    if (Array.isArray(v.edges))
        v.edges = { upsert: v.edges };
    if (Array.isArray(v.stateSlots))
        v.stateSlots = { upsert: v.stateSlots };
    if (Array.isArray(v.evidence))
        v.evidence = { transitions: v.evidence };
    if (typeof v.notes === "string")
        v.notes = v.notes.trim() ? [v.notes] : [];
    if (v.timeAdvance === undefined && v.time !== undefined)
        v.timeAdvance = v.time;
