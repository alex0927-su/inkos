...
import { PlayActionInterpreterAgent, PlaySceneReconcilerAgent, PlaySceneRendererAgent, PlayWorldMutatorAgent, } from "./play-agents.js";
import { createPlayDB } from "./play-db-factory.js";
import { applyPlayMutation, seedPlayGraph } from "./play-reducer.js";
import { PlayStore } from "./play-store.js";
import { createProductionRunSnapshot, writeProductionRunSnapshot, } from "../production/harness.js";
...
export class PlayRunner {
    ...
    async seedOpening(input) {
        ...
        if (finalMutation.blocked || !isOpeningGraphReady(seededGraph)) {
            throw new PlayOpeningSeedError(finalMutation.blockedReason
                || (language === "en"
    ...
    async step(input, options = {}) {
        ...
        if (!rawInput)
            throw new Error("Play input is empty.");
        await this.store.ensureRun(this.options.worldId, this.options.runId);
        ...
        try {
            ...
            const observations = result.mutation.blocked
                ? [{
                        metric: "play-action",
    ...
    async executeStep(rawInput, turn, options) {
        const world = await this.store.loadWorld(this.options.worldId);
        const language = world?.language ?? "zh";
        ...
        try {
            ...
            if (!rollbackSnapshot)
                throw error;
            try {
                await this.store.restoreRunSnapshot(this.options.worldId, this.options.runId, rollbackSnapshot, requireRestorableGraphDB(this.db));
            }
            catch (rollbackError) {
    ...
    async writeRunStatus(turn, status, observations, error) {
    ...
    async regenerateLastTurn(input) {
        ...
        if (!last)
            throw new Error("No Play turn to regenerate.");
        ...
        const previousVariantId = currentGraph
            ? await this.store.saveVariant(this.options.worldId, this.options.runId, last.turn, await this.store.captureRunSnapshot(this.options.worldId, this.options.runId, {
    ...
    async buildContextBrief(sceneBrief, language, world) {
    ...
    async readOptionalProjection(relativePath) {
        try {
...
function isOpeningGraphReady(graph) {
    if (!graph)
        return false;
...
function renderPlayWorldContext(world, language) {
    if (!world)
        return "";
...
function readGraphSnapshot(db) {
    const maybeSnapshot = db.snapshot;
    if (typeof maybeSnapshot !== "function") {
        return null;
    }
    try {
        return maybeSnapshot.call(db);
    }
    catch {
        return null;
    }
}
function requireRestorableGraphDB(db) {
...
function mergePlayMutations(base, supplement) {
    if (isEmptyMutationSupplement(supplement))
        return base;
...
function mergeMutationSummary(base, supplement) {
...
function isEmptyMutationSupplement(mutation) {
...
function renderEntityRoster(entities, language) {
...
function clampRosterText(value) {
...
//# sourceMappingURL=play-runner.js.map