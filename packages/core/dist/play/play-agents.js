...
export class PlayWorldMutatorAgent extends BaseAgent {
    ...
    async proposeMutation(input) {
        ...
        const systemPrompt = await appendPromptPackGuidance(buildWorldMutatorSystemPrompt(language), { promptId: "play.mutator", projectRoot: this.ctx.projectRoot });
        const messages = [
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            try {
                const raw = await chatWithRetry(() => this.submitStructured(messages, WORLD_MUTATION_TOOL, { temperature: 0.25, maxTokens: 4096 }));
...
function mutationFromStructuredResult(raw, turn, actionKind) {
    return withHostMutationIdentity(PlayMutationSchema.parse({
        summary: raw.summary,
...
function withHostMutationIdentity(mutation, turn, actionKind) {
...
function hasMutationResult(mutation) {
...
export class PlaySceneRendererAgent extends BaseAgent {
    ...
    async render(input) {
        const language = input.language ?? "zh";
        const systemPrompt = await appendPromptPackGuidance(buildSceneRendererSystemPrompt(input.mode ?? "open", language), { promptId: "play.renderer", projectRoot: this.ctx.projectRoot });
        const messages = [
            { role: "system", content: systemPrompt },
            { role: "user", content: buildSceneRendererUserPrompt(input, language) },
        ];
        const raw = await chatWithRetry(() => this.submitStructured(messages, PLAY_SCENE_RENDER_TOOL, { temperature: 0.45, maxTokens: 4096 }));
...
export class PlaySceneReconcilerAgent extends BaseAgent {
    ...
    async reconcile(input) {
        ...
        try {
            const raw = await chatWithRetry(() => this.submitStructured(messages, GRAPH_RECONCILIATION_TOOL, { temperature: 0.1, maxTokens: 2048 }));
