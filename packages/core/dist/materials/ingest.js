...
export async function ingestMaterial(projectRoot, input, deps = {}) {
...
async function readMaterialSource(projectRoot, input, deps) {
    if (input.sourceKind === "url") {
        if (!input.url)
            throw new Error("ingest_material.url is required for URL sources.");
        return readUrlMaterial(input.url, deps.fetch ?? fetch);
    }
    if (!input.filePath)
        throw new Error("ingest_material.filePath is required for file sources.");
...
async function readUrlMaterial(url, fetchImpl) {
...
async function extractBufferMaterial(buffer, meta) {
    ...
    if (isPdf(meta.filename, mimeType)) {
        ...
        if (!text)
            throw new Error("PDF text extraction returned no text. Scanned PDFs require OCR and are not supported yet.");
        return {
            kind: "pdf",
            source: meta.source,
...
function isHtml(filename, mimeType) {
    const ext = extname(filename).toLowerCase();
...
function titleFromSource(input) {
    if (input.filename)
        return stripExtension(input.filename);
    if (input.filePath)
        return stripExtension(basename(input.filePath));
    if (!input.url)
        return undefined;
    try {
