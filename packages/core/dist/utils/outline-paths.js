 ...
 */
export async function isBookFoundationComplete(bookDir) {
    const required = [
    ...
    try {
        ...
        if (hasLegacyCharacterMatrixRoles(matrix))
            return true;
    }
    catch {
...
function hasLegacyCharacterMatrixRoles(content) {
 ...
 */
export async function readRoleCards(bookDir) {
...
async function collectRoleDir(dir, tier, out) {
    let entries;
    try {
 ...
 */
export async function readCharacterContext(bookDir, fallbackPlaceholder = "") {
...
export async function readCurrentStateWithFallback(bookDir, fallbackPlaceholder = "") {
    ...
    const parts = ["# 初始状态（第 0 章，由 roles + 种子伏笔派生）"];
