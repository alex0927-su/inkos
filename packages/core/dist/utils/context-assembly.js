...
export function buildGovernedTrace(params) {
    const protectedEntries = params.contextPackage.selectedContext.filter((entry) => isProtectedContextSource(entry.source));
    const compressibleEntries = params.contextPackage.selectedContext.filter((entry) => !isProtectedContextSource(entry.source));
...
export function isProtectedContextSource(source) {
...
function sumContextTokens(entries) {
    return entries.reduce((total, entry) => total + estimateContextSourceTokens(entry), 0);
}
function estimateContextSourceTokens(entry) {
