...
export function analyzeHookHealth(params) {
    ...
    if (activeHooks.length > maxActiveHooks) {
        issues.push(warning(params.language, params.language === "en"
            ? `There are ${activeHooks.length} active hooks, above the recommended cap of ${maxActiveHooks}.`
            : `当前有 ${activeHooks.length} 个活跃伏笔，已经高于建议上限 ${maxActiveHooks} 个。`, params.language === "en"
    ...
    const pressuredHooks = lifecycleEntries.filter(({ hook, lifecycle }) => staleHookIds.has(hook.hookId)
    ...
    if (unresolvedPressure.length > 0) {
        issues.push(warning(params.language, buildPressureDescription({
        ...
        const latestRealAdvance = activeHooks.reduce((max, hook) => Math.max(max, hook.lastAdvancedChapter), 0);
        if (params.noAdvanceWindow !== undefined
            ...
            issues.push(warning(params.language, params.language === "en"
                ...
                : `已经连续 ${params.chapterNumber - latestRealAdvance} 章没有真实伏笔推进。`, params.language === "en"
    ...
    if (params.delta) {
        ...
        if (newHookIds.length >= newHookBurstThreshold && params.delta.hookOps.resolve.length === 0) {
            issues.push(warning(params.language, params.language === "en"
                ...
                : `本章新开了 ${newHookIds.length} 个伏笔，但没有回收任何旧债。`, params.language === "en"
...
function localizePressureLabel(lifecycle, language) {
...
function warning(language, description, suggestion) {
    return {
        severity: "warning",
...
//# sourceMappingURL=hook-health.js.map