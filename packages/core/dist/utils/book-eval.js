...
export function computeChapterEvalScore(ch) {
    let score = 100;
...
function computeHookResolveRate(content) {
    if (!content.trim())
        return 0;
    let totalHooks = 0;
    let resolvedHooks = 0;
    for (const line of content.split("\n")) {
        if (!/^\|.*\|.*\|/.test(line))
            continue;
        if (line.includes("---") || /hook|伏笔/i.test(line))
            continue;
        totalHooks += 1;
        if (/resolved|已回收|已解决/i.test(line))
            resolvedHooks += 1;
    }
