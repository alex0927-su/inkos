...
export function buildGovernedHookWorkingSet(params) {
    ...
    const selectedIds = new Set(params.contextPackage.selectedContext
...
function collectHookAgendaIds(chapterIntent) {
...
export function mergeTableMarkdownByKey(original, updated, keyColumns) {
...
export function mergeCharacterMatrixMarkdown(original, updated) {
...
function parseSingleTable(content) {
...
function pickScaffold(primary, fallback) {
...
function extractCharacterCandidatesFromMatrix(matrixMarkdown) {
    const parsed = parseSections(matrixMarkdown);
    const names = new Set();
...
function filterMatrixSection(sectionBody, sectionIndex, activeNames) {
...
function isNameMentioned(candidate, corpus) {
    if (!candidate || !corpus)
        return false;
...
//# sourceMappingURL=governed-working-set.js.map