...
function tryExactMatch(content, target) {
...
function tryFuzzyMatch(content, target) {
    const normalizedTarget = normalizeWhitespace(target);
    if (normalizedTarget.length < 10)
        return null; // Too short to fuzzy match safely
