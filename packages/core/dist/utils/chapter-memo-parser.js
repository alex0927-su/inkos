...
const GOAL_HEADINGS = ["## 本章目标", "## Chapter goal"];
const THREAD_HEADINGS = ["## 关联线索", "## Thread refs", "## Related threads"];
/**
...
function stripWrappingFence(raw) {
    const trimmed = raw.trim();
    const fenced = trimmed.match(/^```(?:md|markdown)?\s*\n([\s\S]*?)\n```\s*$/i);
    return fenced?.[1]?.trim() ?? trimmed;
}
function dropLeadingProse(raw) {
    const markers = [
        "# 第 ",
        "# Chapter ",
        ...GOAL_HEADINGS,
...
function extractAnyHeading(body, headings) {
...
function extractThreadRefs(body) {
...
function extractMemoBody(markdown) {
...
function makeDisplayGoal(goal) {
    if (goal.length <= 50)
        return goal;
    return `${goal.slice(0, 47).trimEnd()}...`;
}
function prependFullGoalIfNeeded(markdown, body, fullGoal, displayGoal) {
    if (fullGoal === displayGoal)
        return body;
