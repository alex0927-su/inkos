...
describe("guardAssistantMessageStream", () => {
    ...
    it("ends a stream that never produces its first event", async () => {
        const guarded = guardAssistantMessageStream(MODEL, () => createAssistantMessageEventStream(), undefined, { firstEventTimeoutMs: 10 });
        const events = [];
    ...
    it("keeps environment overrides authoritative", async () => {
        ...
        const guarded = guardAssistantMessageStream(MODEL, () => createAssistantMessageEventStream(), undefined, { firstEventTimeoutMs: 60_000 });
        const events = [];
        for await (const event of guarded)
            events.push(event);
        expect(events[0]).toMatchObject({
...
//# sourceMappingURL=provider-stream-deadline.test.js.map