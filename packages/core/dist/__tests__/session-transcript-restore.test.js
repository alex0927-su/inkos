...
import { adaptRestoredAgentMessagesForModel, appendRestoredHistoryBoundary, deriveBookSessionFromTranscript, restoreAgentMessagesFromTranscript, TOOL_RESULT_BRIDGE_TEXT, } from "../interaction/session-transcript-restore.js";
const usage = {
    input: 0,
    output: 0,
    cacheRead: 0,
...
describe("session transcript restore", () => {
    ...
    it("移除最后 assistant message 的 trailing thinking block", async () => {
        ...
        const restored = await restoreAgentMessagesFromTranscript(projectRoot, "s1");
        expect(restored[0].content).toEqual([{ type: "text", text: "回答" }]);
    });
    it("跨模型恢复时移除 provider-specific thinking，但保留正文", () => {
        ...
        expect(adapted[0].content).toEqual([{ type: "text", text: "可见回答" }]);
    });
    it("同模型恢复时保留 thinking continuity", () => {
        ...
        expect(adapted[0].content).toEqual(messages[0].content);
    });
    ...
    it("切到 native Google 时把旧 OpenAI-compatible Gemini 工具回合文本化", () => {
        ...
        const body = JSON.stringify(adapted);
        expect(body).not.toContain("\"toolCall\"");
        expect(adapted.some((message) => message.role === "toolResult")).toBe(false);
        expect(adapted).toEqual([
            expect.objectContaining({
                role: "system",
                content: expect.stringContaining("[Historical tool results]"),
...
//# sourceMappingURL=session-transcript-restore.test.js.map