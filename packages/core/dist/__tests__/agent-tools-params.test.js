...
function withAgentContext(pipeline) {
    return {
        ...pipeline,
...
describe("architect agent — BookConfig construction", () => {
    ...
    beforeEach(() => {
        ...
        const mockPipeline = withAgentContext({ initBook: initBookMock });
...
describe("writer agent — wordCount passthrough", () => {
    let writeNextChapterMock;
    let tool;
    beforeEach(() => {
        writeNextChapterMock = vi.fn(async () => ({ wordCount: 3000 }));
        const mockPipeline = withAgentContext({ writeNextChapter: writeNextChapterMock });
...
describe("auditor agent — rich return value", () => {
    it("returns issue details with severity", async () => {
        ...
        const tool = createSubAgentTool(withAgentContext({ auditDraft: auditDraftMock }), "my-book");
...
describe("reviser agent — mode field", () => {
    ...
    beforeEach(() => {
        ...
        tool = createSubAgentTool(withAgentContext({ reviseDraft: reviseDraftMock }), "my-book");
    });
...
//# sourceMappingURL=agent-tools-params.test.js.map