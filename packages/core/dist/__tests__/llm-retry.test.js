...
describe("isTransientLLMHttpError", () => {
    ...
    it("matches the real aggregator 503 message that aborted whole runs", () => {
        expect(isTransientLLMHttpError(new Error("503 The model provider is temporarily unavailable. Please retry later or contact support."))).toBe(true);
    });
    ...
    it("does NOT retry a 500 / MODEL_NOT_AVAILABLE (model not on inference — retry is futile)", () => {
        ...
        expect(isTransientLLMHttpError(new Error('{"code":500,"reason":"MODEL_NOT_AVAILABLE","message":"model not available"}'))).toBe(false);
    });
});
//# sourceMappingURL=llm-retry.test.js.map