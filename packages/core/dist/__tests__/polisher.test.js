...
describe("PolisherAgent", () => {
    ...
    it("encodes file-layer scope boundary and six prose 雷点 in the zh system prompt", async () => {
        ...
        const chatSpy = vi.spyOn(PolisherAgent.prototype, "chat").mockResolvedValue({
            content: "润色后的正文。",
            usage: ZERO_USAGE,
        });
    ...
    it("returns polished content and flags 'changed' when output differs", async () => {
        const agent = makeAgent();
        vi.spyOn(PolisherAgent.prototype, "chat").mockResolvedValue({
            content: "润色后的正文。",
            usage: ZERO_USAGE,
        });
...
//# sourceMappingURL=polisher.test.js.map