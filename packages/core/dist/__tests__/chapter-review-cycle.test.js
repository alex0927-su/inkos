...
const baseParams = {
    ...
    analyzeSensitiveWords: () => ({ found: [], issues: [] }),
    logWarn: () => undefined,
    logStage: () => undefined,
};
describe("runChapterReviewCycle v9", () => {
    it("feeds postWriteErrors as extra issues into first assessment", async () => {
        ...
        const result = await runChapterReviewCycle({
            ...
            runPostWriteChecks: (content) => content === "b".repeat(200)
                ? [{ severity: "critical", category: "chapter-number-reference", description: "contains chapter ref", suggestion: "remove it" }]
                : [],
        });
    ...
    it("normalizes deterministic surface blockers before audit and repair", async () => {
        ...
        const result = await runChapterReviewCycle({
            ...
            runPostWriteChecks: (content) => content.includes("——")
                ? [{ severity: "critical", category: "禁止破折号", description: "出现了破折号", suggestion: "用逗号断句" }]
                : [],
        });
...
//# sourceMappingURL=chapter-review-cycle.test.js.map