...
import { FoundationReviewerAgent, FoundationReviewParseError, } from "../agents/foundation-reviewer.js";
import { PolisherAgent } from "../agents/polisher.js";
import { MemoryDB } from "../state/memory-db.js";
import * as memoryDbModule from "../state/memory-db.js";
import { countChapterLength } from "../utils/length-metrics.js";
import { listChapterVersions, readChapterVersion, saveChapterUserBrief, } from "../state/chapter-workspace.js";
...
function createWriterOutput(overrides = {}) {
    return {
        chapterNumber: 1,
        title: "Test Chapter",
...
function createReviseOutput(overrides = {}) {
    return {
...
function createAnalyzedOutput(overrides = {}) {
...
function createSettledRevisionOutput(input, overrides = {}) {
...
async function snapshotRevisionBaseline(state, bookId, chapterNumber) {
    const storyDir = join(state.bookDir(bookId), "story");
    await readFile(join(storyDir, "current_state.md"), "utf-8").catch(() => writeFile(join(storyDir, "current_state.md"), createStateCard({
    ...
    await readFile(join(storyDir, "pending_hooks.md"), "utf-8").catch(() => writeFile(join(storyDir, "pending_hooks.md"), "# Pending Hooks\n", "utf-8"));
...
async function createRunnerFixture(configOverrides = {}) {
...
describe("PipelineRunner", () => {
    beforeEach(() => {
        ...
        vi.spyOn(ReviserAgent.prototype, "reviseChapter").mockImplementation(async (_bookDir, chapterContent, _chapterNumber, _issues, _mode, _genre, _options) => createReviseOutput({
    ...
    it("does not reuse override clients when credential sources differ", () => {
        ...
        try {
            ...
            const resolveOverride = runner.resolveOverride.bind(runner);
            ...
            if (previousKeyA === undefined)
                delete process.env.TEST_KEY_A;
            else
                process.env.TEST_KEY_A = previousKeyA;
            if (previousKeyB === undefined)
                delete process.env.TEST_KEY_B;
            else
                process.env.TEST_KEY_B = previousKeyB;
        }
    });
    ...
    it("applies creation-draft overrides while initializing a book", async () => {
        ...
        try {
            ...
            expect(generateFoundationSpy).toHaveBeenCalledWith(book, expect.stringContaining("近未来港口城"), undefined);
            const storyDir = join(root, "books", bookId, "story");
    ...
    it("feeds foundation review feedback into the regeneration call after a rejection", async () => {
        ...
        const generate = vi.fn(async (_reviewFeedback) => foundation);
        ...
        try {
            const result = await runner.generateAndReviewFoundation({
                generate,
                reviewer,
                mode: "original",
                language: "zh",
    ...
    it("honors configured foundation review retry count before accepting a rejected foundation", async () => {
        ...
        try {
            await runner.generateAndReviewFoundation({
                generate,
                reviewer,
                mode: "original",
                language: "zh",
    ...
    it("reuses an existing planned intent for draft when no new context is provided on the governed path", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(state.bookDir(bookId), "story", "story_bible.md"), "# Story Bible\n\n- The jade seal cannot be destroyed.\n", "utf-8"),
            writeFile(join(state.bookDir(bookId), "story", "pending_hooks.md"), "# Pending Hooks\n\n- Why the mentor vanished after the trial.\n", "utf-8"),
            writeFile(join(state.bookDir(bookId), "story", "runtime", "chapter-0001.intent.md"), [
                "# Chapter Intent",
                "",
                "## Goal",
    ...
    sqliteIt("syncs current-state facts into memory.db after drafting a chapter", async () => {
        ...
        try {
            ...
            try {
                expect(memoryDb.getCurrentFacts()).toEqual(expect.arrayContaining([
                ...
                expect(memoryDb.getActiveHooks()).toEqual(expect.arrayContaining([
                    expect.objectContaining({
                        hookId: "mentor-debt",
                        status: "open",
                    }),
    ...
    sqliteIt("syncs narrative memory from structured runtime state instead of stale markdown projections", async () => {
        ...
        try {
            await runner.syncNarrativeMemoryIndex(bookId);
            const memoryDb = new MemoryDB(state.bookDir(bookId));
            try {
    ...
    sqliteIt("retries transient sqlite busy errors during narrative memory sync", async () => {
        ...
        vi.spyOn(memoryDbModule, "MemoryDB").mockImplementation((...args) => {
    ...
    it("routes writeNextChapter through planner and composer on the governed path", async () => {
        ...
        const planChapter = vi.spyOn(PlannerAgent.prototype, "planChapter").mockImplementation(async function (input) {
        ...
        try {
            ...
            expect(writeInput?.contextPackage?.selectedContext.length).toBeGreaterThan(0);
            const run = JSON.parse(await readFile(join(state.bookDir(bookId), "story", "runtime", "chapter-0001.run.json"), "utf-8"));
    ...
    it("re-plans instead of reusing a persisted invalid intent artifact on the governed path", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(storyDir, "volume_outline.md"), [
                "# Volume Outline",
                "",
            ...
            writeFile(join(runtimeDir, "chapter-0001.intent.md"), [
                "# Chapter Intent",
                "",
                "## Goal",
                "**",
    ...
    it("passes reduced control inputs into auditor and reviser on the governed path", async () => {
        ...
        const auditChapter = vi.spyOn(ContinuityAuditor.prototype, "auditChapter").mockResolvedValue(createAuditResult({
            passed: false,
            issues: [CRITICAL_ISSUE],
            summary: "needs revision",
        }));
        const reviseChapter = vi.spyOn(ReviserAgent.prototype, "reviseChapter").mockResolvedValue(createReviseOutput({
    ...
    it("records a length warning when repair still misses the hard range", async () => {
        ...
        try {
            ...
            expect(result.lengthWarnings?.[0]).toContain("未达到篇幅预算");
    ...
    it("degrades to state-degraded when state validation errors instead of aborting", async () => {
        ...
        vi.spyOn(StateValidatorAgent.prototype, "validate").mockRejectedValue(new Error("LLM returned empty response"));
    ...
    it("retries settlement after state contradictions without rewriting the chapter body", async () => {
        ...
        const writeSpy = vi.spyOn(WriterAgent.prototype, "writeChapter").mockResolvedValue(createWriterOutput({
        ...
        const settleSpy = vi.spyOn(WriterAgent.prototype, "settleChapterState").mockResolvedValue(createWriterOutput({
            content: "Healthy chapter body with the copper token in his coat.",
            wordCount: "Healthy chapter body with the copper token in his coat.".length,
        ...
        const result = await runner.writeNextChapter(bookId, countChapterLength("Healthy chapter body with the copper token in his coat.", "zh_chars"));
        expect(result.status).toBe("ready-for-review");
    ...
    it("repairs the latest state-degraded chapter from persisted body without rewriting it", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(bookDir, "chapters", "0001_Broken_Persistence.md"), "# 第1章 Broken Persistence\n\nHealthy chapter body with the copper token in his coat.", "utf-8"),
            state.saveChapterIndex(bookId, [{
                    number: 1,
                    title: "Broken Persistence",
                    status: "state-degraded",
    ...
    it("syncs the latest edited chapter body back into truth files without requiring state-degraded status", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(baselineDir, "current_state.md"), "baseline state", "utf-8"),
            writeFile(join(baselineDir, "pending_hooks.md"), "baseline hooks", "utf-8"),
            writeFile(join(bookDir, "chapters", "0001_夜灯.md"), "# 第1章 夜灯\n\n林越推门进去，先停在门槛外听了一息，再去看柜台后那盏没关的灯。", "utf-8"),
            state.saveChapterIndex(bookId, [{
                    number: 1,
                    title: "夜灯",
                    status: "approved",
        ...
        const result = await runner.resyncChapterArtifacts(bookId, 1);
        const savedIndex = await state.loadChapterIndex(bookId);
        expect(result.status).toBe("ready-for-review");
        expect(result.chapterNumber).toBe(1);
        expect(settleSpy).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("still persists the chapter when the state validator appends markdown after a valid JSON verdict", async () => {
        ...
        vi.spyOn(ReviserAgent.prototype, "reviseChapter").mockImplementation(async (_bookDir, chapterContent) => createReviseOutput({
            revisedContent: chapterContent,
            wordCount: chapterContent.length,
        }));
        const { root, runner, state, bookId } = await createRunnerFixture({});
        ...
        vi.spyOn(StateValidatorAgent.prototype, "chat").mockResolvedValue({
            content: [
    ...
    it("preserves imported chapter body when the analyzer only returns truth-file updates", async () => {
        ...
        await writeFile(join(chaptersDir, "0002_旧标题.md"), "# 第2章 旧标题\n\n旧正文不应该在重导入后继续留在目录里。", "utf-8");
        const importedBody = [
    ...
    sqliteIt("rebuilds fact history from imported chapter snapshots", async () => {
        ...
        try {
            ...
            try {
                ...
                expect(chapterOneFacts).toEqual(expect.arrayContaining([
                    expect.objectContaining({
                        predicate: "Current Conflict",
                        object: "The mentor debt is still personal.",
                    }),
                ]));
                expect(currentFacts).toEqual(expect.arrayContaining([
                    expect.objectContaining({
                        predicate: "Current Conflict",
    ...
    sqliteIt("rebuilds fact history from structured snapshot state instead of stale markdown snapshots", async () => {
        ...
        await Promise.all([
            writeFile(join(snapshotOneDir, "current_state.md"), createStateCard({
                chapter: 1,
        ...
        try {
            await runner.syncCurrentStateFactHistory(bookId, 1);
            const memoryDb = new MemoryDB(state.bookDir(bookId));
            try {
                expect(memoryDb.getCurrentFacts()).toEqual(expect.arrayContaining([
                    expect.objectContaining({
                ...
                expect(memoryDb.getCurrentFacts()).not.toEqual(expect.arrayContaining([
                    expect.objectContaining({
    ...
    it("does not leak imported future state into early replay chapters", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(storyDir, "snapshots", "0", "current_state.md"), createStateCard({
                chapter: 60,
            ...
            writeFile(join(storyDir, "snapshots", "0", "pending_hooks.md"), [
                "# Pending Hooks",
                "",
                "| hook_id | start_chapter | type | status | last_advanced_chapter | expected_payoff | notes |",
                "| --- | --- | --- | --- | --- | --- | --- |",
    ...
    it("feeds hook health warnings back into pipeline audit and dedicated drift guidance", async () => {
        ...
        try {
            ...
            expect(persistedChapter?.auditIssues).toEqual(expect.arrayContaining([
    ...
    it("passes one-off external brief into manual revise on the governed path", async () => {
        ...
        await saveChapterUserBrief(state.bookDir(bookId), 1, "保留证人关于雨夜账本的原话。");
        vi.spyOn(ContinuityAuditor.prototype, "auditChapter")
            .mockResolvedValueOnce(createAuditResult({
            passed: false,
            issues: [CRITICAL_ISSUE],
    ...
    async function createRevisionGateFixture(revisionGate) {
    ...
    it("keeps chapter and truth files unchanged when revised-body settlement cannot validate", async () => {
        ...
        vi.spyOn(ContinuityAuditor.prototype, "auditChapter").mockResolvedValueOnce(createAuditResult({ passed: false, issues: [CRITICAL_ISSUE], summary: "needs revision" }));
        vi.spyOn(StateValidatorAgent.prototype, "validate").mockResolvedValue({
            passed: false,
            repairRequired: true,
            warnings: [{
    ...
    it("runs an explicit rework even when the current chapter already passes audit", async () => {
        ...
        try {
            const result = await runner.reviseDraft(bookId, 1, "rework", "保留事实，但重新组织整章冲突。");
            const savedChapter = await readFile(join(chaptersDir, "0001_Test_Chapter.md"), "utf-8");
            expect(result.applied).toBe(true);
            expect(savedChapter).toContain(revisedBody);
        }
    ...
    it("keeps current truth intact and marks downstream chapters when reworking an older chapter", async () => {
        ...
        await Promise.all([
            writeFile(join(chaptersDir, "0002_Later_Chapter.md"), "# 第2章 Later Chapter\n\n第二章已经发生。", "utf-8"),
        ...
        vi.spyOn(ContinuityAuditor.prototype, "auditChapter")
            ...
            .mockResolvedValueOnce(createAuditResult({ passed: true, issues: [], summary: "clean alternative" }));
        try {
            const result = await runner.reviseDraft(bookId, 1, "rework", "重写第一章，但不要假装第二章尚未发生。");
            const savedChapter = await readFile(join(chaptersDir, "0001_Test_Chapter.md"), "utf-8");
            const savedIndex = await state.loadChapterIndex(bookId);
            expect(result.applied).toBe(true);
            expect(savedChapter).toContain(revisedBody);
    ...
    it("excludes pure sequence-level fatigue from revision blocker counts", async () => {
        ...
        const result = await runner.evaluateMergedAudit({
            auditor: {
                auditChapter: vi.fn().mockResolvedValue(createAuditResult({
                    passed: true,
                    issues: [],
                    summary: "clean",
                })),
