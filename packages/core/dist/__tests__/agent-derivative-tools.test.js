...
import { createContinuationImportTool, createFanficBookTool, createImitationBookTool, createSpinoffBookTool, } from "../agent/agent-tools.js";
...
describe("derivative-work agent tools", () => {
    ...
    it("creates fanfiction from confirmed source text inside the agent context", async () => {
        ...
        const tool = createFanficBookTool(pipeline, root);
        ...
        expect(pipeline.initFanficBook).toHaveBeenCalledWith(expect.objectContaining({
            id: "霜港来信",
            title: "霜港来信",
            fanficMode: "canon",
            targetChapters: 24,
        }), "原作中林鹿守着一座废弃灯塔。", "霜港正典", "canon");
    ...
    it("inherits parent-book defaults when creating a side story", async () => {
        const pipeline = mockPipeline();
        const tool = createSpinoffBookTool(pipeline, root);
        const result = await tool.execute("spinoff-1", {
            title: "雨夜旧账",
            parentBookId: "harbor",
            direction: "老船工失踪前最后一夜",
        });
        expect(pipeline.initSpinoffBook).toHaveBeenCalledWith(expect.objectContaining({
            id: "雨夜旧账",
            parentBookId: "harbor",
            platform: "tomato",
            genre: "suspense",
    ...
    it("creates an original imitation project without copying the reference plot", async () => {
        const pipeline = mockPipeline();
        const tool = createImitationBookTool(pipeline, root);
        ...
        expect(pipeline.initImitationBook).toHaveBeenCalledWith(expect.objectContaining({ id: "纸灯新案", title: "纸灯新案" }), "雨从檐角一滴滴落下来，像一只迟疑的钟。", "县城档案员调查一批被替换的死亡证明", "参考散文");
        expect(result.details).toMatchObject({
            kind: "book_created",
    ...
    it("imports an uploaded manuscript into a newly created continuation book", async () => {
        ...
        await writeFile(join(root, ".inkos", "uploads", "continuation", "novel.txt"), "第一章 雨港\n\n林鹿在旧码头找到一本账簿。\n\n第二章 空号\n\n电话那头只有潮声。\n", "utf-8");
        const pipeline = mockPipeline();
...
//# sourceMappingURL=agent-derivative-tools.test.js.map