...
import { BookConfigSchema, PlatformSchema, GenreSchema, BookStatusSchema, normalizePlatformId, normalizePlatformOrOther, } from "../models/book.js";
...
import { ChapterIntentSchema, ChapterMemoSchema, ContextPackageSchema, RuleStackSchema, ChapterTraceSchema, } from "../models/input-governance.js";
import { LengthSpecSchema, LengthTelemetrySchema, LengthWarningSchema, } from "../models/length-governance.js";
import { RuntimeStateDeltaSchema, StateManifestSchema, HooksStateSchema, ChapterSummariesStateSchema, CurrentStateStateSchema, } from "../models/runtime-state.js";
// ---------------------------------------------------------------------------
// BookConfig
// ---------------------------------------------------------------------------
...
describe("ChapterIntentSchema", () => {
    ...
    it("rejects invalid chapter numbers", () => {
        expect(() => ChapterIntentSchema.parse({
            chapter: 0,
            goal: "Bad chapter",
        })).toThrow();
    });
...
describe("ChapterMemoSchema", () => {
    ...
    it("rejects goal longer than 50 chars", () => {
        expect(() => ChapterMemoSchema.parse({
            chapter: 1,
            goal: "a".repeat(51),
            body: "## 当前任务\nx",
        })).toThrow();
...
describe("ContextPackageSchema", () => {
    ...
    it("rejects context entries without source", () => {
        expect(() => ContextPackageSchema.parse({
            chapter: 8,
            selectedContext: [
                {
...
describe("Runtime state schemas", () => {
    ...
    it("rejects natural-language numeric drift in runtime-state delta hooks", () => {
        expect(() => RuntimeStateDeltaSchema.parse({
            chapter: 12,
            hookOps: {
                upsert: [
                    {
