...
import { buildRuntimeStateArtifacts, loadNarrativeMemorySeed, loadRuntimeStateSnapshot, loadRuntimeStateSnapshotAtChapter, loadSnapshotCurrentStateFacts, } from "../state/runtime-state-store.js";
describe("runtime-state-store memory helpers", () => {
    ...
    it("normalizes dormant and confirmed hook lifecycle terms from markdown projections", async () => {
        ...
        await mkdir(chaptersDir, { recursive: true });
        await Promise.all([
            writeFile(join(chaptersDir, "index.json"), JSON.stringify([{ number: 1, title: "Ch1", status: "approved" }]), "utf-8"),
            writeFile(join(storyDir, "pending_hooks.md"), [
                "| hook_id | start_chapter | type | status | last_advanced | expected_payoff | notes |",
                "| --- | --- | --- | --- | --- | --- | --- |",
        ...
        expect(snapshot.hooks.hooks).toEqual(expect.arrayContaining([
    ...
    it("prefers structured snapshot state over stale markdown snapshots for fact history rebuild", async () => {
        ...
        await Promise.all([
            writeFile(join(snapshotDir, "current_state.md"), [
                "# Current State",
                "",
                "| Field | Value |",
                "| --- | --- |",
    ...
    it("reconstructs a pre-chapter runtime snapshot from markdown without losing stable hook ids", async () => {
        ...
        await Promise.all([
            ...
            writeFile(join(snapshotDir, "pending_hooks.md"), [
                "| hook_id | start_chapter | type | status | last_advanced | expected_payoff | notes |",
                "| --- | --- | --- | --- | --- | --- | --- |",
    ...
    it("rejects persisted duplicate summary chapters in structured runtime state", async () => {
        ...
        await mkdir(stateDir, { recursive: true });
        await mkdir(chaptersDir, { recursive: true });
        await writeFile(join(chaptersDir, "index.json"), JSON.stringify(Array.from({ length: 12 }, (_, i) => ({ number: i + 1, title: `Ch${i + 1}`, status: "approved" }))), "utf-8");
        await Promise.all([
            writeFile(join(stateDir, "manifest.json"), JSON.stringify({
                schemaVersion: 2,
                language: "zh",
    ...
    it("repairs persisted hooks with empty type instead of failing the library load", async () => {
        ...
        await mkdir(chaptersDir, { recursive: true });
        await Promise.all([
            writeFile(join(chaptersDir, "index.json"), JSON.stringify(Array.from({ length: 5 }, (_, i) => ({ number: i + 1, title: `Ch${i + 1}`, status: "approved" }))), "utf-8"),
            writeFile(join(storyDir, "pending_hooks.md"), [
    ...
    it("canonicalizes structurally valid new hook candidates without semantic guessing", async () => {
        ...
        await mkdir(stateDir, { recursive: true });
        await mkdir(chaptersDir, { recursive: true });
        await writeFile(join(chaptersDir, "index.json"), JSON.stringify(Array.from({ length: 11 }, (_, i) => ({ number: i + 1, title: `Ch${i + 1}`, status: "approved" }))), "utf-8");
        await Promise.all([
            writeFile(join(stateDir, "manifest.json"), JSON.stringify({
                schemaVersion: 2,
                language: "en",
...
//# sourceMappingURL=runtime-state-store.test.js.map