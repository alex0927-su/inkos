...
describe("book reference bindings", () => {
    ...
    it("stores one project asset and only a purpose binding inside the target book", async () => {
        ...
        const bindingText = await readFile(join(root, "books", bookId, "story", "reference_bindings.json"), "utf-8");
    ...
    it("lists resolved assets, surfaces missing assets, and unbinds without deleting the asset", async () => {
        ...
        await writeFile(join(root, "books", bookId, "story", "reference_bindings.json"), JSON.stringify({
            version: 1,
            bookId,
            bindings: [
...
describe("book reference context selection", () => {
    ...
    it("lets the model select full sections from bound assets only", async () => {
        ...
        const selector = vi.fn(async (request) => {
...
async function createReferenceAsset(root, title, content) {
...
//# sourceMappingURL=book-references.test.js.map