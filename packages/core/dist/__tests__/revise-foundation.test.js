...
const buildArchitect = () => new ArchitectAgent({
    client: TEST_CLIENT,
    model: "test-model",
...
describe("architect generateFoundation with reviseFrom option", () => {
    ...
    it("injects legacy content into the system prompt when reviseFrom is supplied", async () => {
        ...
        const systemMsg = (chatSpy.mock.calls[0]?.[0])[0];
...
describe("pipeline.reviseFoundation", () => {
    ...
    it("Phase 5 书二次 revise 时从 outline/roles 权威源读（不把 shim 喂给 architect）", async () => {
        ...
        try {
            ...
            // outline/ 是权威，内容完整
            await writeFile(join(bookDir, "story", "outline", "story_frame.md"), "## 主题与基调\n完整的段落式世界观描写，包含所有设定细节、人物关系、剧情主线。" + "a".repeat(5000), "utf-8");
            await writeFile(join(bookDir, "story", "outline", "volume_map.md"), "## 段 1\n完整卷大纲描写。" + "b".repeat(5000), "utf-8");
            // roles/ 是权威，内容完整
            await writeFile(join(bookDir, "story", "roles", "主要角色", "林辞.md"), "## 核心标签\n冷静、执着\n\n## 主角线\n完整角色线，3000 字描写，反映复杂内在位移" + "c".repeat(3000), "utf-8");
            // story_bible.md / character_matrix.md 是 shim（只有指针和摘录）
            await writeFile(join(bookDir, "story", "story_bible.md"), "# 故事圣经（已废弃）\n\n> 权威来源是 outline/story_frame.md\n\n## story_frame 摘录\n\n只有前 2000 字...", "utf-8");
            await writeFile(join(bookDir, "story", "character_matrix.md"), "# 角色矩阵（已废弃）\n\n> 权威来源是 roles/ 目录\n\n## 主要角色\n\n- roles/主要角色/林辞.md", "utf-8");
...
//# sourceMappingURL=revise-foundation.test.js.map