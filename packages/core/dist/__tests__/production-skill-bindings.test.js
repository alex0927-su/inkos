...
import { NON_LONG_PRODUCTION_CAPABILITIES, PRODUCTION_SKILL_IDS, mergeActivatedSkillGuidance, resolveProductionSkillActivations, } from "../skills/production-bindings.js";
function skill(id) {
    return {
        id,
        name: id,
...
describe("production skill bindings", () => {
    ...
    it("resolves host-selected skills and lets project replacements win", () => {
        ...
        const replacement = { ...builtin, source: "project", body: "project play method" };
        const resolved = resolveProductionSkillActivations([builtin, replacement, skill("inkos-long-writing")], "play");
    ...
    it("merges default and user-requested skills without duplicates", () => {
        ...
        const replacement = {
            skill: { ...defaultActivation.skill, source: "project", body: "replacement" },
            resources: [],
        };
        expect(mergeActivatedSkillGuidance([defaultActivation], [userActivation, replacement])).toEqual([replacement, userActivation]);
    });
    it("retrieves task-relevant references for production workers", async () => {
        ...
        try {
            ...
            const [resolved] = await hydrateActivatedSkillGuidance([activation], "Write a confrontation where every dialogue reply changes leverage.") ?? [];
...
//# sourceMappingURL=production-skill-bindings.test.js.map