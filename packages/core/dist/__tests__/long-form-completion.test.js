...
describe("long-form completion", () => {
    ...
    it("does not resume an interrupted transport response", async () => {
        const generate = vi.fn().mockRejectedValue(new PartialResponseError("不完整内容", new Error("connection closed")));
    ...
    it("runs one recovery pass after output-limit continuation fragments are merged", async () => {
        const generate = vi.fn()
            .mockRejectedValueOnce(new PartialResponseError("# 雨夜\n\n## 人物\n店员", new Error("model reached the output limit"), "output-limit"))
            .mockResolvedValueOnce({
            content: "# 雨夜\n\n## 人物\n店员\n\n## 剧本正文\n完整正文",
            usage: { promptTokens: 1, completionTokens: 1, totalTokens: 2 },
        });
        const recoverAfterContinuation = vi.fn().mockResolvedValue("# 雨夜\n\n## 人物\n店员\n\n## 剧本正文\n完整正文");
...
//# sourceMappingURL=long-form-completion.test.js.map