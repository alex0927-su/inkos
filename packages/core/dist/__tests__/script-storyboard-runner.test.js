...
describe("storyboard creation runner", () => {
    ...
    beforeEach(async () => {
        ...
        generateStoryGraphMock.mockImplementation((_client, _model, input, options) => {
    ...
    it("writes a first-class image asset manifest and asset directories", async () => {
        ...
        const manifest = JSON.parse(await readFile(join(root, result.assetsManifestPath), "utf-8"));
    ...
    it("continues a script after a confirmed model output limit before committing it", async () => {
        chatCompletionMock.mockReset();
        chatCompletionMock.mockRejectedValueOnce(new PartialResponseError("# 监控里没有他\n\n## 剧本正文\n\n便利店。暴雨。\n陌生人推门。", new Error("model reached the output limit (length)"), "output-limit"));
        ...
        const continuationMessages = chatCompletionMock.mock.calls[1]?.[2];
        ...
        const recoveryMessages = chatCompletionMock.mock.calls[2]?.[2];
    ...
    it("generates large episodic storyboards in complete structural segments", async () => {
        ...
        for (const [index, call] of chatCompletionMock.mock.calls.entries()) {
            ...
            if (index > 0)
                expect(prompt).not.toContain("第一集完整正文");
            if (index < 2)
                expect(prompt).not.toContain("第三集完整正文");
        }
    ...
    it("subdivides oversized episodes by explicit Markdown scene structure without dropping source", async () => {
        ...
        const prompts = chatCompletionMock.mock.calls.map((call) => call[2][1].content);
    ...
    it("writes interactive-film story tree, flags, script, storyboard, prompts, and image assets", async () => {
        ...
        if (!graph)
            throw new Error("Expected generated story graph");
    ...
    it("runs storyboard creation in English with English prompts and parses English section headings", async () => {
        ...
        await expect(readFile(join(root, result.specPath), "utf-8")).resolves.toContain("# Cold Ledger Storyboard Creation Spec");
        const manifest = JSON.parse(await readFile(join(root, result.assetsManifestPath), "utf-8"));
        expect(manifest.assets.map((asset) => [asset.shotId, asset.prompt])).toEqual([
    ...
    it("runs interactive-film creation in English and splits the package by English headings", async () => {
        ...
        expect(user).not.toMatch(/[一-鿿]/);
        await expect(readFile(join(root, result.specPath), "utf-8")).resolves.toContain("Interactive Film Creation Spec");
        ...
        await expect(readFile(join(root, result.imagePromptsPath), "utf-8")).resolves.toContain("medieval banquet hall");
        const graph = await loadStoryGraph(root, "crown-feast");
        expect(graph).not.toBeNull();
        if (!graph)
            throw new Error("Expected fallback story graph");
...
function makeRuntime(root, activatedSkills) {
    return {
        projectRoot: root,
        model: "test-model",
        client: {
...
//# sourceMappingURL=script-storyboard-runner.test.js.map