...
describe("ComposerAgent", () => {
    ...
    it("adds semantically selected book references as traceable compressible context", async () => {
        ...
        const reference = result.contextPackage.selectedContext.find((entry) => entry.source === "reference/material-1#relationship-arc");
    ...
    it("preserves later author-intent constraints instead of reducing them to the first line", async () => {
        await writeFile(join(storyDir, "author_intent.md"), [
            "# Author Intent",
            "",
            "标题：《桥洞来信》",
            "",
    ...
    it("preserves later canon constraints from file context instead of first-line excerpts", async () => {
        await writeFile(join(storyDir, "parent_canon.md"), [
            "# Parent Canon",
            "",
            "档案编号：旧城案",
            "",
        ...
        const parentCanonEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/parent_canon.md");
    ...
    it("writes trace output describing planner inputs and selected sources", async () => {
        ...
        expect(result.trace.tokenBudget.totalSelectedTokens).toBeGreaterThanOrEqual(result.trace.tokenBudget.protectedTokens);
    ...
    it("compiles only compressible context when selected context exceeds budget", async () => {
        ...
        const authorIntent = result.contextPackage.selectedContext.find((entry) => entry.source === "story/author_intent.md");
        const compiled = result.contextPackage.selectedContext.find((entry) => entry.source === "runtime/compiled-compressible-context");
    ...
    it("fails loudly when protected context alone exceeds the input budget", async () => {
        await writeFile(join(storyDir, "author_intent.md"), `# Author Intent\n\n${"protected author intent ".repeat(5000)}`, "utf-8");
    ...
    it("selects relevant legacy outline sections instead of protecting whole legacy files", async () => {
        ...
        const storyBibleEntry = result.contextPackage.selectedContext.find((entry) => entry.source.startsWith("story/story_bible.md#"));
        const volumeEntry = result.contextPackage.selectedContext.find((entry) => entry.source.startsWith("story/volume_outline.md#"));
    ...
    it("selects relevant outline sections instead of protecting whole large outline files", async () => {
        ...
        expect(result.trace.contextTiers.protectedSources.some((source) => source.startsWith("story/outline/story_frame.md#"))).toBe(true);
        expect(result.trace.contextTiers.protectedSources.some((source) => source.startsWith("story/outline/volume_map.md#"))).toBe(true);
    });
    ...
    it("adds current-state fact evidence retrieved from sqlite-backed memory", async () => {
        ...
        const factEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/current_state.md#current-conflict");
    ...
    it("adds relevant volume-summary evidence for long-span retrieval after consolidation", async () => {
        await writeFile(join(storyDir, "volume_summaries.md"), [
            "# Volume Summaries",
            "",
            "## Volume 1 (Ch.1-40)",
            "",
        ...
        const volumeEntry = result.contextPackage.selectedContext.find((entry) => entry.source.startsWith("story/volume_summaries.md#"));
    ...
    it("adds explicit title history, mood trail, and canon evidence for governed writing", async () => {
        ...
        const titleEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/chapter_summaries.md#recent_titles");
        const moodEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/chapter_summaries.md#recent_mood_type_trail");
        const parentCanonEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/parent_canon.md");
        const fanficCanonEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/fanfic_canon.md");
    ...
    it("includes dedicated audit drift guidance instead of relying on current_state pollution", async () => {
        ...
        const driftEntry = result.contextPackage.selectedContext.find((entry) => entry.source === "story/audit_drift.md");
