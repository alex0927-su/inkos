...
import { normalizeHookId, parsePendingHooksMarkdown, renderHookSnapshot, } from "../utils/story-markdown.js";
...
// ---------------------------------------------------------------------------
function buildPhase7Response(language = "zh") {
...
describe("Phase 7 hotfix 2 — reviewer gates critical severity on promoted", () => {
    it("zh reviewer prompt references 升级=是 as critical gate and the 已阻 N 章 token", async () => {
        ...
        try {
            await writeFile(join(bookDirLocal, "book.json"), JSON.stringify({
                id: "hf-zh",
                title: "hotfix-zh",
                genre: "urban",
                platform: "other",
            ...
            vi.spyOn(ContinuityAuditor.prototype, "chatWithSearch").mockImplementation(stub);
            vi.spyOn(ContinuityAuditor.prototype, "chat").mockImplementation(stub);
            await auditor.auditChapter(bookDirLocal, "章节正文。", 1, "urban");
            const messages = stub.mock.calls[0]?.[0];
...
//# sourceMappingURL=phase7-hotfix.test.js.map