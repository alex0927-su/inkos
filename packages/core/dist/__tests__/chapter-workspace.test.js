...
import { archiveChapterVersion, listChapterVersions, readChapterPlanDocument, readChapterUserBrief, readChapterVersion, saveChapterUserBrief, } from "../state/chapter-workspace.js";
...
describe("chapter workspace", () => {
    ...
    it("archives immutable chapter versions and lists newest first", async () => {
        const bookDir = await mkdtemp(join(tmpdir(), "inkos-chapter-workspace-"));
        const first = await archiveChapterVersion(bookDir, 4, "# 第4章 初稿\n\n旧正文。", "manual", new Date("2026-07-01T00:00:00.000Z"));
        const second = await archiveChapterVersion(bookDir, 4, "# 第4章 二稿\n\n新正文。", "revision", new Date("2026-07-02T00:00:00.000Z"));
    ...
    it("does not expose archives from another chapter", async () => {
        const bookDir = await mkdtemp(join(tmpdir(), "inkos-chapter-workspace-"));
        const version = await archiveChapterVersion(bookDir, 1, "# 第1章", "regeneration", new Date("2026-07-03T00:00:00.000Z"));
        await expect(readChapterVersion(bookDir, 2, version.id)).rejects.toThrow();
        await expect(readFile(join(bookDir, "chapters", ".versions", "0001", `${version.id}.md`), "utf-8")).resolves.toBe("# 第1章");
    });
});
//# sourceMappingURL=chapter-workspace.test.js.map