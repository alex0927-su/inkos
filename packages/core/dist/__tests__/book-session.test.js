...
import { BookSessionSchema, GlobalSessionSchema, createBookSession, appendBookSessionMessage, } from "../interaction/session.js";
describe("BookSession", () => {
    ...
    describe("appendBookSessionMessage", () => {
        ...
        it("sorts messages by timestamp", () => {
            ...
            session = appendBookSessionMessage(session, { role: "user", content: "second", timestamp: 200 });
            session = appendBookSessionMessage(session, { role: "assistant", content: "first", timestamp: 100 });
            expect(session.messages[0].content).toBe("first");
            expect(session.messages[1].content).toBe("second");
        });
    });
});
//# sourceMappingURL=book-session.test.js.map