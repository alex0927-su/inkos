...
import { loadBookSession, persistBookSession, listBookSessions, renameBookSession, deleteBookSession, migrateBookSession, createAndPersistBookSession, extractFirstUserMessageTitle, SessionAlreadyMigratedError, } from "../interaction/book-session-store.js";
import { createBookSession, appendBookSessionMessage } from "../interaction/session.js";
import { appendManualSessionMessages, legacyBookSessionPath, readTranscriptEvents, sessionsDir, } from "../interaction/session-transcript.js";
...
describe("book-session-store", () => {
    ...
    describe("persistBookSession + loadBookSession", () => {
        ...
        it("persists messages", async () => {
            ...
            session = appendBookSessionMessage(session, { role: "user", content: "test", timestamp: 100 });
            await persistBookSession(tempDir, session);
            const loaded = await loadBookSession(tempDir, session.sessionId);
        ...
        it("读取 legacy JSON 时迁移为 JSONL 并保留 UI thinking", async () => {
            const session = {
                ...
                messages: [
                    ...
                    { role: "assistant", content: "好的", thinking: "旧思考", timestamp: 11 },
                ],
            };
            await persistBookSession(tempDir, session);
        ...
        it("does not duplicate legacy messages when migration runs concurrently", async () => {
            ...
            const withMessages = {
                ...legacy,
                messages: [
                    { role: "user", content: "old user", timestamp: legacy.createdAt },
                    { role: "assistant", content: "old assistant", timestamp: legacy.createdAt + 1 },
                ],
            };
            await mkdir(sessionsDir(tempDir), { recursive: true });
            await writeFile(legacyBookSessionPath(tempDir, "legacy-race"), JSON.stringify(withMessages, null, 2), "utf-8");
    ...
    describe("listBookSessions: 老 session lazy migration", () => {
        it("把 title 为 null 但已有用户消息的老 session 补写 title 并持久化", async () => {
            const session = {
                ...
                title: null,
                messages: [
                    { role: "user", content: "帮我写下一章", timestamp: 100 },
                    { role: "assistant", content: "好的，正在构思...", timestamp: 200 },
                ],
            };
            await persistBookSession(tempDir, session);
            // 触发 list → 应该迁移
        ...
        it("不覆盖已有 title 的 session", async () => {
            ...
            session = {
                ...
                title: "原有标题",
                messages: [
                    { role: "user", content: "后来的消息", timestamp: 100 },
                ],
            };
            await persistBookSession(tempDir, session);
            const list = await listBookSessions(tempDir, "book-a");
        ...
        it("多条老 session 同时迁移", async () => {
            const s1 = {
                ...
                title: null,
                messages: [{ role: "user", content: "问题一", timestamp: 1 }],
            };
            const s2 = {
                ...createBookSession("book-b"),
                title: null,
                messages: [{ role: "user", content: "问题二", timestamp: 1 }],
            };
            await persistBookSession(tempDir, s1);
            await persistBookSession(tempDir, s2);
    ...
    describe("migrateBookSession", () => {
        ...
        it("throws when session is already bound to a book", async () => {
            ...
            await persistBookSession(tempDir, session);
            await expect(migrateBookSession(tempDir, session.sessionId, "book-2")).rejects.toBeInstanceOf(SessionAlreadyMigratedError);
        });
    });
});
//# sourceMappingURL=book-session-store.test.js.map