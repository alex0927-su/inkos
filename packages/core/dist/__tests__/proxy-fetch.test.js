...
const proxyAgentMock = vi.fn((url) => ({ kind: "proxy-agent", url }));
vi.mock("undici", () => ({
...
describe("proxy fetch helpers", () => {
    ...
    it("prefers explicit llm proxyUrl over environment proxy variables", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.example/v1/chat/completions", expect.objectContaining({
            method: "POST",
    ...
    it("uses INKOS_LLM_PROXY_URL before standard HTTPS_PROXY/HTTP_PROXY env vars", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.example/v1/models", expect.objectContaining({
    ...
    it("does not attach a dispatcher when no proxy is configured", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("https://api.example/v1/models", { headers: { Authorization: "Bearer test" } });
    });
});
//# sourceMappingURL=proxy-fetch.test.js.map