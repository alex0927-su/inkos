...
import { buildPlayEntityImagePrompt, buildPlaySceneImagePrompt, readPlayImageManifest, setPlayImageEntry, playImageFileName, readPlayImageSettings, writePlayImageSettings, DEFAULT_PLAY_IMAGE_SETTINGS, } from "../play/play-image.js";
describe("play image prompts", () => {
    it("frames an actor as an entity image and anchors it to the world premise without forcing a style", () => {
        const prompt = buildPlayEntityImagePrompt({ type: "actor", label: "林深", summary: "急诊科医生，能读濒死者的记忆" }, "都市超自然：触碰濒死者会读到对方一段记忆，代价是丢失自己的记忆。");
    ...
    it("includes user-defined world and visual contracts without assuming RPG tiers", () => {
        const prompt = buildPlayEntityImagePrompt({ type: "item", label: "旧瓷杯", summary: "合租屋餐桌上总被推到玩家座位旁边。" }, {
            premise: "雨夜合租屋关系本。",
...
//# sourceMappingURL=play-image.test.js.map