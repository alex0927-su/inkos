...
import { createPlayStartTool, createPlayReviseTool, createPlayStepTool, } from "../agent/agent-tools.js";
...
function createReadyPlayStartTool(root, sessionId, playMode, options = {}) {
...
describe("agent play tools", () => {
    ...
    it("runs opening seeding inside the abort scope and does not swallow user cancellation", async () => {
        ...
        const runWithAgentContext = vi.fn(async (context, task) => {
        ...
        const seedOpening = vi.fn(async (_input) => null);
        const tool = createPlayStartTool(pipeline, root, sessionId, undefined, {
            runnerFactory: ({ db }) => ({
                seedOpening: async (...args) => {
                    seedReadyGraph(db);
                    return seedOpening(...args);
        ...
        expect(runWithAgentContext).toHaveBeenCalledWith({ signal: controller.signal, activatedSkills: [] }, expect.any(Function));
        const cancelledSessionId = "1700000000000-abort2";
        const cancelled = new AbortController();
        cancelled.abort();
        const cancelledTool = createPlayStartTool(pipeline, root, cancelledSessionId);
    ...
    it("runs Play inside its own professional Skill context", async () => {
        ...
        expect(pipeline.runWithAgentContext).toHaveBeenCalledWith({ signal: undefined, activatedSkills: [playSkill] }, expect.any(Function));
        expect(result.details).toMatchObject({
            kind: "play_turn_advanced",
...
//# sourceMappingURL=agent-play-tools.test.js.map