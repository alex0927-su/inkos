...
vi.mock("../agent/pi-stream.js", async () => {
    const actual = await vi.importActual("../agent/pi-stream.js");
...
describe("Pi worker harness", () => {
    ...
    it("aborts the current worker and never starts the next serial step", async () => {
        ...
        chatCompletionMock.mockImplementationOnce(async (_client, _model, _messages, options) => new Promise((_resolve, reject) => {
    ...
    it("submits host-consumed state through one typed Pi tool call", async () => {
        ...
        guardedPiStreamMock.mockImplementation((model) => {
        ...
        const result = await runWorkerAgentTool(client(), "deepseek-v4-flash", [{ role: "user", content: "登记当前角色状态" }], {
            name: "submit_state",
            label: "提交状态",
            description: "提交角色状态。",
...
//# sourceMappingURL=worker-agent.test.js.map