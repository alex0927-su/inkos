...
describe("dispatchNotification telegram formats", () => {
    it("markdown format keeps parse_mode and bold title", async () => {
        await dispatchNotification([{ type: "telegram", botToken: "123:ABC", chatId: "-100", format: "markdown" }], message);
    ...
    it("text format omits parse_mode and strips markdown marks", async () => {
        await dispatchNotification([{ type: "telegram", botToken: "123:ABC", chatId: "-100", format: "text" }], message);
        const body = lastRequestBody();
...
describe("dispatchNotification wechat-work formats", () => {
    it("markdown format keeps msgtype markdown", async () => {
        await dispatchNotification([{ type: "wechat-work", webhookUrl: "https://qyapi.weixin.qq.com/hook", format: "markdown" }], message);
        const body = lastRequestBody();
    ...
    it("text format sends msgtype text with plain content", async () => {
        await dispatchNotification([{ type: "wechat-work", webhookUrl: "https://qyapi.weixin.qq.com/hook", format: "text" }], message);
        const body = lastRequestBody();
...
describe("dispatchNotification feishu formats", () => {
    it("markdown format keeps the interactive card", async () => {
        await dispatchNotification([{ type: "feishu", webhookUrl: "https://open.feishu.cn/hook", format: "markdown" }], message);
        const body = lastRequestBody();
    ...
    it("text format sends msg_type text with title and plain body combined", async () => {
        await dispatchNotification([{ type: "feishu", webhookUrl: "https://open.feishu.cn/hook", format: "text" }], message);
        const body = lastRequestBody();
...
describe("dispatchNotification webhook format", () => {
    it("includes the channel format in the JSON payload data", async () => {
        await dispatchNotification([{ type: "webhook", url: "https://example.com/hook", events: [], format: "text" }], message);
        const body = lastRequestBody();
...
//# sourceMappingURL=notify-format.test.js.map