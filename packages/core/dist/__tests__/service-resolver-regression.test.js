...
describe("resolveServiceModel regression — preset baseUrl override", () => {
    ...
    beforeEach(async () => {
        ...
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { minimax: { apiKey: "sk-minimax-test" } } }));
    });
    afterEach(async () => {
    ...
    it("customBaseUrl overrides both preset and pi-ai", async () => {
        const result = await resolveServiceModel("minimax", "MiniMax-M2.7", root, "https://custom-proxy.example.com/v1");
        expect(result.model.baseUrl).toBe("https://custom-proxy.example.com/v1");
    });
});
//# sourceMappingURL=service-resolver-regression.test.js.map