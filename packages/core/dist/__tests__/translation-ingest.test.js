...
async function writeMinimalEpub(path) {
    const zip = new JSZip();
...
describe("translation ingestion", () => {
    ...
    it("creates a persistent translation project from Markdown chapters", async () => {
        await writeFile(join(root, "inputs", "source.md"), [
            "# 第一章 雨夜",
            "",
            "第一段很短。",
            "",
        ...
        const persisted = await readJson(join(root, created.manifestPath));
        expect(persisted.id).toBe(created.manifest.id);
        const firstChapter = await readJson(join(root, created.manifest.chapters[0].sourcePath));
    ...
    it("creates a translation project from plain TXT", async () => {
        await writeFile(join(root, "inputs", "source.txt"), [
            "第一段没有 Markdown 标题。",
            "",
            "第二段仍然需要作为翻译段落保留。",
        ].join("\n"));
        ...
        const chapter = await readJson(join(root, created.manifest.chapters[0].sourcePath));
...
//# sourceMappingURL=translation-ingest.test.js.map