...
describe("loadProjectConfig local provider auth", () => {
    ...
    it("loads service-based config using defaultModel and project secrets", async () => {
        ...
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { moonshot: { apiKey: "sk-moon" } } }, null, 2), "utf-8");
    ...
    it("derives provider/baseUrl from the MiniMax preset single source of truth", async () => {
        ...
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { minimax: { apiKey: "sk-minimax" } } }, null, 2), "utf-8");
        const config = await loadProjectConfig(root);
    ...
    it("loads custom service config using custom secret key and entry baseUrl", async () => {
        ...
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { "custom:内网GPT": { apiKey: "sk-corp" } } }, null, 2), "utf-8");
        const config = await loadProjectConfig(root);
