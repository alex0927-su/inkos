...
describe("ArchitectAgent", () => {
    ...
    it("uses English prompts when generating foundation from imported English chapters", async () => {
        ...
        await agent.generateFoundationFromImport(book, "Chapter 1: Prelude\n\nA cold wind crossed the harbor.");
    ...
    it("embeds reviewer feedback into fanfic foundation regeneration prompts", async () => {
        ...
        await agent.generateFanficFoundation(book, "# 原作正典\n- 罗辑在面壁计划中留下了一处空档。", "canon", "请明确分岔点，并用原创冲突替代原作重走。");
        const messages = chat.mock.calls[0]?.[0];
        expect(messages[0]?.content).toContain("上一轮审核反馈");
    ...
    const buildPhase5Agent = () => new ArchitectAgent({
        client: {
            provider: "openai",
            apiFormat: "chat",
            stream: false,
...
//# sourceMappingURL=architect.test.js.map