...
describe("Scheduler", () => {
    ...
    it("does not start a second write cycle while one is still running", async () => {
        ...
        let releaseCycle;
        const blockedCycle = new Promise((resolve) => {
            releaseCycle = resolve;
        });
        const runWriteCycle = vi
            .spyOn(scheduler, "runWriteCycle")
        ...
        vi.spyOn(scheduler, "runRadarScan")
    ...
    it("treats state-degraded chapter results as handled failures", async () => {
        ...
        vi.spyOn(scheduler.pipeline, "writeNextChapter").mockResolvedValue({
            chapterNumber: 3,
            title: "Broken State",
            wordCount: 2100,
            revised: false,
        ...
        const handleAuditFailure = vi.spyOn(scheduler, "handleAuditFailure").mockResolvedValue(undefined);
        const success = await scheduler.writeOneChapter("book-1", bookConfig);
