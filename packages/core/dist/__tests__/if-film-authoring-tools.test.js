...
describe("direct-write authoring tools", () => {
    ...
    it("set_world_anchor applies a delta and persists worldAnchor", async () => {
        ...
        const res = await tool.execute("call-1", { storyCore: "查账复仇", theme: "信任" });
        expect(res.content[0].text).toMatch(/world|锚点|updated|rev/i);
    ...
    it("add_variable applies a delta and persists the variable", async () => {
        ...
        await tool.execute("call-2", { name: "trust", type: "counter", default: 0, desc: "信任" });
    ...
    it("define_ending persists an ending referencing an existing node", async () => {
        ...
        expect(res.content[0].text).toMatch(/美好结局|Ending|defined/i);
...
//# sourceMappingURL=if-film-authoring-tools.test.js.map