...
import { makeModelBranch, snapshotCanonicalFiles, writeForecastFixtureBook, } from "./helpers/forecast-fixture.js";
...
function runtime(projectRoot) {
    return { client: { provider: "openai" }, model: "fake", projectRoot };
}
...
describe("narrative forecast runner", () => {
    ...
    it("leaves no forecast files behind when the model output is invalid", async () => {
        const chatSpy = vi.spyOn(NarrativeForecastAgent.prototype, "chat").mockResolvedValue({ content: "不是 JSON", usage: { promptTokens: 0, completionTokens: 0, totalTokens: 0 } });
    ...
    it("refuses to select a branch that does not exist", async () => {
        ...
        expect(await exists(join(bookDir, "story", "runtime", "narrative-forecasts", FIXED_ID, "selected-branch-plan.md"))).toBe(false);
    });
...
//# sourceMappingURL=forecast-runner.test.js.map