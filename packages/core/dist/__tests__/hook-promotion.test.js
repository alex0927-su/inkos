...
import { defaultHalfLifeChapters, resolveHalfLifeChapters, shouldPromoteHook, } from "../utils/hook-promotion.js";
...
describe("shouldPromoteHook — Phase 7 four-rule promotion", () => {
    ...
    it("reads advancedCount from context when hook itself has none", () => {
        ...
        const decision = shouldPromoteHook(hook, makeContext({
    ...
    it("combines multiple reasons when several conditions hold", () => {
        ...
        const decision = shouldPromoteHook(hook, makeContext({ volumeBoundaries: VOL_MAP }));
        expect(decision.promote).toBe(true);
        expect(new Set(decision.reasons)).toEqual(new Set(["core_hook", "depends_on", "advanced_count", "cross_volume"]));
    });
...
//# sourceMappingURL=hook-promotion.test.js.map