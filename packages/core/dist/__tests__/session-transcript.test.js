...
import { appendManualSessionMessages, appendTranscriptEvents, appendTranscriptEvent, nextTranscriptSeq, readTranscriptEvents, transcriptPath, } from "../interaction/session-transcript.js";
...
describe("session transcript codec", () => {
    ...
    it("跳过坏行并保留合法 event", async () => {
        ...
        await writeFile(join(dir, "s1.jsonl"), [
            JSON.stringify({
                type: "request_started",
                version: 1,
                sessionId: "s1",
    ...
    it("atomically assigns unique seq for concurrent generated events", async () => {
        await Promise.all(Array.from({ length: 10 }, (_, index) => appendTranscriptEvents(projectRoot, "s-concurrent", ({ nextSeq }) => [{
                type: "request_started",
                version: 1,
...
//# sourceMappingURL=session-transcript.test.js.map