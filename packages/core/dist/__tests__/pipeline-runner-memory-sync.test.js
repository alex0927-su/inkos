...
describe("PipelineRunner structured-state memory sync", () => {
    ...
    it("uses structured runtime state for narrative memory during writeNextChapter even when markdown projections drift after persistence", async () => {
        ...
        vi.spyOn(ComposerAgent.prototype, "selectMemoryCandidates").mockImplementation(async (request) => request.candidates.map((candidate) => candidate.id));
        vi.spyOn(ComposerAgent.prototype, "selectOutlineSections").mockImplementation(async (request) => request.candidates.map((candidate) => candidate.source));
        vi.spyOn(ComposerAgent.prototype, "selectReferenceSections").mockImplementation(async (request) => request.candidates.map((candidate) => candidate.source));
        ...
        vi.spyOn(WriterAgent.prototype, "saveChapter").mockImplementation(async function (bookDirArg, output, numericalSystem, language) {
            await originalSaveChapter.call(this, bookDirArg, output, numericalSystem, language);
            await Promise.all([
                writeFile(join(bookDirArg, "story", "pending_hooks.md"), [
                    "| hook_id | start_chapter | type | status | last_advanced | expected_payoff | notes |",
                    "| --- | --- | --- | --- | --- | --- | --- |",
                    "| markdown-drift-hook | 1 | mystery | open | 1 | 5 | Drifted markdown hook |",
                    "",
                ].join("\n"), "utf-8"),
                writeFile(join(bookDirArg, "story", "chapter_summaries.md"), [
...
//# sourceMappingURL=pipeline-runner-memory-sync.test.js.map