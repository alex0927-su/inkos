...
import { createReadTool, createGenerateCoverTool, createSubAgentTool, createShortFictionRunTool, createPatchChapterTextTool, createReplaceChapterTextTool, createResyncChapterStateTool, createDeleteLatestChapterTool, createPlayEditTool, createPlayStartTool, createProposeActionTool, createRenameEntityTool, createScriptCreationTool, createStoryboardCreationTool, createInteractiveFilmCreationTool, createManageBookReferenceTool, createWriteFileTool, createWriteTruthFileTool, } from "../agent/agent-tools.js";
...
describe("agent deterministic writing tools", () => {
    ...
    it("deletes only the latest chapter through the deterministic tool path", async () => {
        ...
        await expect(readFile(join(state.bookDir("harbor"), "chapters", ".trash", "0003_Storm.md"), "utf-8")).resolves.toContain("jade seal");
    });
    ...
    it("resyncs derived chapter state and returns the fresh audit result without rewriting prose", async () => {
        ...
        const tool = createResyncChapterStateTool(pipeline, "harbor", { language: "en" });
    ...
    it("requires an explicit title when the architect sub-agent creates a book", async () => {
        ...
        const tool = createSubAgentTool(pipeline, null);
    ...
    it("uses the confirmed Play scene as the execution source of truth", async () => {
        ...
        const tool = createPlayStartTool(pipeline, root, "play-session-truncated", "open", {
            actionPayload: {
                playStart: {
                    title: "旧戏院夜巡",
                    premise: "我在县城旧戏院做夜间检修，停电后舞台下传来拍板声。",
    ...
    it("does not emit a confirmation card when the proposed action payload is invalid", async () => {
        ...
        })).rejects.toThrow("Invalid proposed action payload");
    });
    ...
    it("uses confirmed create-book payload when architect tool args drift or omit defaults", async () => {
        ...
        const tool = createSubAgentTool(pipeline, null, undefined, {
            actionPayload: {
                createBook: {
                    title: "夜间派送",
                    genre: "urban",
    ...
    it("returns an architect incomplete result instead of throwing when foundation repair fails", async () => {
        const pipeline = contextPipeline({
            initBook: vi.fn(async () => {
                throw new ArchitectIncompleteFoundationError(["roles", "pending_hooks"], "=== SECTION: story_frame ===\n已有世界观草稿", "基础设定没有生成完整。");
            }),
        });
        const tool = createSubAgentTool(pipeline, null);
    ...
    it("passes chapterWordCount through the writer sub-agent", async () => {
        ...
        const tool = createSubAgentTool(pipeline, "harbor");
        ...
        expect(pipeline.writeNextChapter).toHaveBeenCalledWith("harbor", 2600, undefined, "继续写，控制在 2600 字");
    });
    it("runs a requested chapter batch through one writer operation", async () => {
        ...
        expect(pipeline.writeChapters).toHaveBeenCalledWith("harbor", 5, expect.objectContaining({ wordCount: 2600 }));
    ...
    it("runs the writer pipeline inside the shared AgentContext scope", async () => {
        ...
        expect(pipeline.runWithAgentContext).toHaveBeenCalledWith({ signal: controller.signal, activatedSkills: [] }, expect.any(Function));
    ...
    it("passes activated Skill guidance and the exact user instruction into the writer", async () => {
        ...
        const tool = createSubAgentTool(pipeline, "harbor", undefined, {
        ...
        expect(pipeline.runWithAgentContext).toHaveBeenCalledWith({ signal: undefined, activatedSkills }, expect.any(Function));
    ...
    it("injects the host-selected long-writing Skill into the worker without relying on agent intent", async () => {
        ...
        expect(pipeline.runWithAgentContext).toHaveBeenCalledWith({ signal: undefined, activatedSkills: [longWritingSkill] }, expect.any(Function));
        expect(result.details).toMatchObject({
            kind: "chapter_written",
    ...
    it("uses the active book for writer when bookId is omitted", async () => {
        ...
        expect(pipeline.writeNextChapter).toHaveBeenCalledWith("harbor", 2600, undefined, "继续写下一章");
    });
    it("uses structured exporter arguments instead of parsing natural-language instruction", async () => {
        const tool = createSubAgentTool({}, "harbor", root);
    ...
    it("documents sub_agent bookId as an optional active-book override", () => {
        const tool = createSubAgentTool({}, "harbor");
    ...
    it("exposes script, storyboard, and interactive-film creation as standalone production tools", () => {
        ...
        const scriptTool = createScriptCreationTool(pipeline, root);
        const storyboardTool = createStoryboardCreationTool(pipeline, root);
        const interactiveFilmTool = createInteractiveFilmCreationTool(pipeline, root);
    ...
    it("uses explicit exporter params instead of guessing from instruction", async () => {
        const pipeline = {};
        const tool = createSubAgentTool(pipeline, "harbor", root);
