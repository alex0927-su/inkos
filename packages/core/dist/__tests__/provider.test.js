...
import { __resetFixedTemperatureWarnings, chatCompletion, } from "../llm/provider.js";
...
vi.mock("@mariozechner/pi-ai", async (importOriginal) => {
    ...
    return {
        ...original,
        streamSimple: (...args) => mockStreamSimple(...args),
        completeSimple: (...args) => mockCompleteSimple(...args),
...
function makeErrorStream(message) {
    return {
        [Symbol.asyncIterator]() {
            return {
                async next() {
...
function makeClient(temperature = 0.7, extra = {}) {
    return {
        provider: "openai",
        service: "openai",
        configSource: "studio",
...
async function captureError(task) {
    try {
        await task;
    }
    catch (error) {
...
describe("chatCompletion via pi-ai", () => {
    ...
    it("throws when stream produces no text content", async () => {
        ...
        const client = makeClient();
        const error = await captureError(chatCompletion(client, "test-model", [{ role: "user", content: "ping" }]));
    ...
    it("passes temperature and maxTokens to streamSimple", async () => {
        ...
        const opts = mockStreamSimple.mock.calls[0]?.[2];
    ...
    it("honors a caller-specific first-event deadline", async () => {
        ...
        const error = await captureError(chatCompletion(makeClient(), "test-model", [{ role: "user", content: "hi" }], {
    ...
    it("rejects non-ASCII API keys before native custom fetch builds headers", async () => {
        ...
        const error = await captureError(chatCompletion(client, "gpt-5.4", [{ role: "user", content: "ping" }]));
...
describe("chatCompletion fixed-temperature clamp (thinking models)", () => {
    ...
    it("clamps per-call temperature override (0.3) to 1 for kimi-k2.5", async () => {
        ...
        await chatCompletion(client, "kimi-k2.5", [{ role: "user", content: "hi" }], { temperature: 0.3 });
        const opts = mockStreamSimple.mock.calls[0]?.[2];
        expect(opts.temperature).toBe(1);
    });
    ...
    it("leaves regular models untouched (no clamp, no warning)", async () => {
        const client = makeClient(0.7);
        const warn = vi.spyOn(console, "warn").mockImplementation(() => { });
        await chatCompletion(client, "moonshot-v1-32k", [{ role: "user", content: "hi" }], { temperature: 0.3 });
        const opts = mockStreamSimple.mock.calls[0]?.[2];
        expect(opts.temperature).toBe(0.3);
...
describe("stream interruption detection", () => {
    ...
    it("retries a pi-ai stream that errors after a long partial instead of silently keeping the truncation", async () => {
        ...
        mockStreamSimple
            .mockReturnValueOnce(partialThenError)
            .mockReturnValueOnce(makeTextStream("重试后的完整内容"));
    ...
    it("treats a pi-ai stream that ends without a done event as interrupted and retries", async () => {
        ...
        mockStreamSimple
            ...
            .mockReturnValueOnce(makeTextStream("第二次完整"));
        const result = await chatCompletion(makeClient(), "test-model", [{ role: "user", content: "写" }]);
    ...
    it("rejects a pi-ai done message whose stop reason is length", async () => {
        ...
        const lengthMessage = { ...partial, stopReason: "length" };
