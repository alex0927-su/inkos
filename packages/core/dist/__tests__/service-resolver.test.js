...
describe("resolveServiceModel", () => {
    ...
    it("resolves built-in service with key from secrets", async () => {
        ...
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { moonshot: { apiKey: "sk-moon" } } }));
    ...
    it("resolves deepseek with correct temperature", async () => {
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { deepseek: { apiKey: "sk-deep" } } }));
    ...
    it("resolves Google to native google-generative-ai model", async () => {
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { google: { apiKey: "sk-google" } } }));
    ...
    it("resolves LM Studio local models without an API key", async () => {
        const result = await resolveServiceModel("lmstudio", "openai/gpt-oss-20b", root);
        expect(result.apiKey).toBe("");
    ...
    it("resolves local custom OpenAI-compatible services without an API key", async () => {
        const result = await resolveServiceModel("custom:LocalProxy", "gpt-5.5", root, "http://127.0.0.1:4567/v1");
        expect(result.apiKey).toBe("");
    ...
    it("resolves custom service with baseUrl", async () => {
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { "custom:内网GPT": { apiKey: "sk-corp" } } }));
        const result = await resolveServiceModel("custom:内网GPT", "gpt-4o", root, "https://llm.internal.corp/v1");
    ...
    it("resolves custom service with responses api format", async () => {
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { "custom:内网GPT": { apiKey: "sk-corp" } } }));
        const result = await resolveServiceModel("custom:内网GPT", "gpt-5.4", root, "https://llm.internal.corp/v1", "responses");
        expect(result.apiKey).toBe("sk-corp");
    ...
    it("resolves MiniMax using the OpenAI-compatible preset endpoint", async () => {
        await mkdir(join(root, ".inkos"), { recursive: true });
        await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { minimax: { apiKey: "sk-minimax" } } }));
...
//# sourceMappingURL=service-resolver.test.js.map