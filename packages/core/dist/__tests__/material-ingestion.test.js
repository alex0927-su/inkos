...
describe("material ingestion", () => {
    ...
    it("archives a project text file as traceable markdown material", async () => {
        ...
        const manifest = JSON.parse(await readFile(join(root, asset.manifestPath), "utf-8"));
        expect(manifest.markdownPath).toBe(asset.markdownPath);
    });
    it("extracts and archives HTML fetched from a URL", async () => {
        const fetchImpl = async () => new Response("<html><head><title>旧账资料</title><style>x{}</style></head><body><h1>冷库流程</h1><script>bad()</script><p>入库单需要签字。</p></body></html>", { status: 200, headers: { "content-type": "text/html; charset=utf-8" } });
        const asset = await ingestMaterial(root, {
            sourceKind: "url",
...
//# sourceMappingURL=material-ingestion.test.js.map