...
describe("StateValidatorAgent", () => {
    ...
    it("accepts a valid JSON object even when the model appends markdown with extra braces", async () => {
        ...
        await expect(agent.validate("Chapter body.", 3, "old state", "new state", "old hooks", "new hooks", "en")).resolves.toEqual({
            warnings: [],
            passed: true,
    ...
    it("returns a structured repair verdict without classifying warning text in code", async () => {
        ...
        await expect(agent.validate("林舟抵达码头。", 3, "位置：家中", "位置：车站", "H1 未推进", "H1 未推进")).resolves.toEqual({
            passed: false,
            repairRequired: true,
            warnings: [{
    ...
    it("passes maxTokens large enough for thinking models to chat()", async () => {
        ...
        const chatSpy = vi.spyOn(agent, "chat").mockResolvedValue({ content: "PASS", usage: ZERO_USAGE });
    ...
    it("passes authority truth context into the cross-file validation prompt", async () => {
        ...
        const chatSpy = vi.spyOn(agent, "chat").mockResolvedValue({ content: "PASS", usage: ZERO_USAGE });
        await agent.validate("正文确认：第五条规则才是天黑后不准出宿舍。", 2, "old state", "new state: 第一条规则已被批注", "old hooks", "new hooks", "zh", {
    ...
    it("does not silently truncate chapter or authority context before validation", async () => {
        ...
        const chatSpy = vi.spyOn(agent, "chat").mockResolvedValue({ content: "PASS", usage: ZERO_USAGE });
        await agent.validate(`${"正文".repeat(7000)}\nCHAPTER_TAIL_MARKER`, 8, "old state", "new state", "old hooks", "new hooks", "zh", {
    ...
    it("throws when the validator model returns an empty response", async () => {
        ...
        await expect(agent.validate("Chapter body.", 3, "old state", "new state", "old hooks", "new hooks", "en")).rejects.toThrow("empty response");
    });
});
//# sourceMappingURL=state-validator-agent.test.js.map