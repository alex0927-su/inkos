...
describe("StateManager", () => {
    ...
    describe("getNextChapterNumber", () => {
        ...
        it("uses durable story progress when chapter index lags behind persisted chapter files", async () => {
            ...
            await Promise.all([
                ...
                writeFile(join(chaptersDir, "0003_Lantern_Vault.md"), "# Chapter 3: Lantern Vault\n\nPersisted body.", "utf-8"),
        ...
        it("ignores non-contiguous poisoned chapter numbers when calculating the next chapter", async () => {
            ...
            await Promise.all([
                ...Array.from({ length: 12 }, (_, index) => writeFile(join(chaptersDir, `${String(index + 1).padStart(4, "0")}_Ch${index + 1}.md`), `# Chapter ${index + 1}\n\nStable body.`, "utf-8")),
                writeFile(join(chaptersDir, "0142_Poisoned.md"), "# Chapter 142\n\nPoisoned body.", "utf-8"),
                writeFile(join(storyDir, "current_state.md"), [
                    "# Current State",
                    "",
                    "| Field | Value |",
                ...
                writeFile(join(storyDir, "chapter_summaries.md"), [
                    ...
                    "| --- | --- | --- | --- | --- | --- | --- | --- |",
                    ...Array.from({ length: 12 }, (_, index) => `| ${index + 1} | Ch${index + 1} | Lin Yue | Event ${index + 1} | Shift ${index + 1} | Hook ${index + 1} | tense | mainline |`),
    ...
    describe("snapshotState / restoreState", () => {
        ...
        it("copies structured runtime state into snapshot/state when present", async () => {
            ...
            await manager.snapshotState(bookId, 1);
            const snapshotManifest = await readFile(join(manager.bookDir(bookId), "story", "snapshots", "1", "state", "manifest.json"), "utf-8");
        ...
        it("removes live optional truth files that are absent from the snapshot", async () => {
            ...
            await manager.snapshotState(bookId, 1);
            await writeFile(join(storyDir, "particle_ledger.md"), "# Ledger added after snapshot", "utf-8");
            const restored = await manager.restoreState(bookId, 1);
            expect(restored).toBe(true);
        ...
        it("rewrite chapter 2 then getNextChapterNumber returns 2", async () => {
            ...
            const mkEntry = (n) => ({
                number: n, title: `Ch${n}`, status: "approved", wordCount: 100,
                createdAt: "", updatedAt: "", auditIssues: [], lengthWarnings: [],
            });
    ...
    describe("ensureControlDocuments", () => {
        it("creates author intent, current focus, and runtime directory", async () => {
            await manager.ensureControlDocuments("control-book", "# Initial Brief\n\nKeep the focus on mentor conflict.\n");
        ...
        it("does not treat future hook start chapters as lastAppliedChapter during bootstrap", async () => {
            ...
            const manifest = JSON.parse(await readFile(join(manager.stateDir(bookId), "manifest.json"), "utf-8"));
        ...
        it("does not treat narrative digits inside hook markdown as runtime chapter progress during bootstrap", async () => {
            ...
            const manifest = JSON.parse(await readFile(join(manager.stateDir(bookId), "manifest.json"), "utf-8"));
            const hooks = JSON.parse(await readFile(join(manager.stateDir(bookId), "hooks.json"), "utf-8"));
    ...
    describe("rollbackToChapter", () => {
        ...
        async function setupRollbackBook() {
