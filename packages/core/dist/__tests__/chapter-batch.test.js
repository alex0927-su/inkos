...
function chapter(chapterNumber, status = "ready-for-review") {
    return {
        chapterNumber,
...
describe("PipelineRunner.writeChapters", () => {
    ...
    it("releases the book lock when an in-flight chapter is aborted", async () => {
        ...
        const release = vi.fn(async () => undefined);
        const acquireBookLock = vi.fn(async () => release);
        const writeLocked = vi.fn(() => new Promise((_resolve, reject) => {
            controller.signal.addEventListener("abort", () => reject(controller.signal.reason), { once: true });
        }));
        const internals = runner;
        internals.state = { acquireBookLock };
        internals._writeNextChapterLocked = writeLocked;
        const pending = runner.runWithAbortSignal(controller.signal, () => runner.writeChapters("demo-book", 5));
...
//# sourceMappingURL=chapter-batch.test.js.map