...
describe("secrets", () => {
    ...
    describe("loadSecrets", () => {
        ...
        it("reads existing secrets file", async () => {
            ...
            await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { moonshot: { apiKey: "sk-test" } } }));
            const secrets = await loadSecrets(root);
    ...
    describe("saveSecrets", () => {
        ...
        it("overwrites existing secrets file", async () => {
            await mkdir(join(root, ".inkos"), { recursive: true });
            await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { old: { apiKey: "old-key" } } }));
            await saveSecrets(root, {
    ...
    describe("getServiceApiKey", () => {
        it("returns key from secrets.json first", async () => {
            await mkdir(join(root, ".inkos"), { recursive: true });
            await writeFile(join(root, ".inkos", "secrets.json"), JSON.stringify({ services: { moonshot: { apiKey: "sk-from-file" } } }));
