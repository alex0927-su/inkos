...
export function makeForecastBranch(overrides = {}) {
    return {
        branchId: "branch-1",
        title: "主角接受提议",
 ...
 */
export async function snapshotCanonicalFiles(bookDir) {
    const snapshot = new Map();
    await walk(bookDir, bookDir, snapshot);
    return snapshot;
}
async function walk(root, dir, snapshot) {
    const entries = await readdir(dir, { withFileTypes: true });
    for (const entry of entries) {
        const path = join(dir, entry.name);
        const rel = relative(root, path);
        if (rel.startsWith(join("story", "runtime", "narrative-forecasts")))
            continue;
...
export function makeForecast(overrides = {}) {
    return {
        version: 1,
...
//# sourceMappingURL=forecast-fixture.js.map