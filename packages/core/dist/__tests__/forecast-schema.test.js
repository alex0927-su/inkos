...
import { FORECAST_MAX_BRANCHES, FORECAST_MIN_BRANCHES, NarrativeForecastSchema, parseForecastModelOutput, } from "../forecast/schema.js";
import { makeForecast, makeForecastBranch } from "./helpers/forecast-fixture.js";
function modelBranches(count) {
...
describe("narrative forecast schema", () => {
    ...
    it("rejects more than the maximum branch count", () => {
        const branches = Array.from({ length: 6 }, (_, index) => makeForecastBranch({ branchId: `branch-${index + 1}` }));
    ...
    it("rejects unknown risk kinds", () => {
        const forecast = makeForecast({
            branches: [
                makeForecastBranch({ risks: [{ kind: "vibes", description: "不合法" }] }),
                makeForecastBranch({ branchId: "branch-2" }),
            ],
        });
        expect(() => NarrativeForecastSchema.parse(forecast)).toThrow();
...
//# sourceMappingURL=forecast-schema.test.js.map