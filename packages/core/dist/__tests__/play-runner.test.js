...
class FakePlayDB {
    ...
    expireEdge(edgeId, validUntilEventId) {
        ...
        if (edge)
            this.edges.set(edgeId, { ...edge, validUntilEventId });
    }
...
describe("PlayRunner", () => {
    ...
    it("runs one player action end to end and persists event, transcript, and projections", async () => {
        ...
        const renderSpy = vi.fn(async (_input) => render);
        ...
        const renderInput = renderSpy.mock.calls[0]?.[0];
    ...
    it("does not commit state when scene rendering fails", async () => {
        ...
        const runner = new PlayRunner({
            ...
            agents: {
                actionInterpreter: {
                    interpret: vi.fn(async () => ({ actionKind: "look", intent: "检查封条" })),
                },
                worldMutator: {
                    proposeMutation: vi.fn(async () => ({
                        eventId: "evt-1",
                        ...
                        summary: "玩家发现封条有新划痕。",
                        entities: {
                            upsert: [{ id: "evidence_seal", type: "evidence", label: "封条划痕" }],
                        },
                    })),
                },
                sceneRenderer: {
    ...
    it("reconciles opening prose into the graph when the opening mutator returns no facts", async () => {
        ...
        const reconcile = vi.fn(async () => ({
            ...
            entities: {
                upsert: [
                    { id: "actor_player", type: "actor", label: "公交司机", summary: "对十名乘客负责。", updatedEventId: "evt-0" },
                    { id: "actor_mother", type: "actor", label: "母亲", summary: "抱着哮喘儿童。", updatedEventId: "evt-0" },
                    { id: "actor_child", type: "actor", label: "哮喘儿童", summary: "缺的是他的哮喘药。", updatedEventId: "evt-0" },
                    { id: "actor_elder", type: "actor", label: "老人", summary: "需要取回自己的胰岛素。", updatedEventId: "evt-0" },
                ],
            },
            edges: {
                upsert: [{
    ...
    it("feeds the world premise and existing entity roster to the mutator so it can reuse ids", async () => {
        ...
        let mutatorContext = "";
        const proposeMutation = vi.fn(async (input) => {
            mutatorContext = input.context;
            return mutation;
        });
        const renderSpy = vi.fn(async (_input) => ({ sceneText: "老陈指节一顿，算盘珠子碰出一声脆响。", suggestedActions: [] }));
        const runner = new PlayRunner({
            projectRoot: root,
    ...
    it("regenerates the latest turn from a checkpoint and keeps both variants", async () => {
        ...
        let version = "A";
        const renderInputs = [];
        const renderReplay = vi.fn(async (input) => {
        ...
        const replayRenderInput = renderInputs[1];
...
//# sourceMappingURL=play-runner.test.js.map