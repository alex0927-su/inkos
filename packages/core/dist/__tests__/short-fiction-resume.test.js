...
import { ShortFictionOutlineAgent, ShortFictionOutlineReviewerAgent, ShortFictionOutlineReviserAgent, ShortFictionWriterAgent, ShortFictionDraftReviewerAgent, ShortFictionDraftReviserAgent, ShortFictionPackagingAgent, parseShortFictionBatchDraft, } from "../agents/short-fiction.js";
...
describe("short fiction resume + failure marker (C2)", () => {
    ...
    it("keeps completing a draft when the first continuation fills only some missing middle chapters", async () => {
        ...
        expect(finalJson.chapters.every((chapter) => chapter.content.length > 0)).toBe(true);
    });
...
function findEmptyChapterNumbers(draft) {
    return draft.chapters.filter((chapter) => !chapter.content.trim()).map((chapter) => chapter.number);
}
//# sourceMappingURL=short-fiction-resume.test.js.map