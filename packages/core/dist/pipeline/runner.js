...
import { ComposerAgent, composeGovernedChapter, contextBudgetFromClient } from "../agents/composer.js";
...
function mergeChapterRevisionInstructions(persistedBrief, currentInstruction) {
 ...
 */
export function buildSpinoffFoundationContext(parentCanon, direction, language) {
...
export class PipelineRunner {
    ...
    async runWithAgentContext(context, task) {
    ...
    async generateAndReviewFoundation(params) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            try {
                ...
                if (!(error instanceof FoundationReviewParseError))
                    throw error;
     ...
     */
    async initSpinoffBook(book, parentBookId, direction) {
        const bookDir = this.state.bookDir(book.id);
        const stageLanguage = await this.resolveBookLanguage(book);
        this.logStage(stageLanguage, { zh: "保存书籍配置", en: "saving book config" });
        await this.state.saveBookConfig(book.id, book);
     ...
     * silently skipped.
     */
    async initImitationBook(book, referenceText, storyIdea, sourceName) {
    ...
    async writeDraft(bookId, context, wordCount) {
        ...
        try {
            ...
            const totalUsage = output.tokenUsage ?? {
                promptTokens: 0,
                completionTokens: 0,
                totalTokens: 0,
            };
    ...
    async reviseDraft(bookId, chapterNumber, mode = DEFAULT_REVISE_MODE, externalContext) {
        const releaseLock = await this.state.acquireBookLock(bookId);
        try {
            const book = await this.state.loadBookConfig(bookId);
            const bookDir = this.state.bookDir(bookId);
            ...
            const effectiveExternalContext = mergeChapterRevisionInstructions(persistedChapterBrief, externalContext ?? this.config.externalContext);
            const reviseControlInput = await this.createGovernedArtifacts(book, bookDir, targetChapter, effectiveExternalContext, { reuseExistingIntentWhenContextMissing: true });
            ...
            const explicitRevisionRequested = Boolean(effectiveExternalContext?.trim())
                || mode === "rewrite"
                || mode === "rework";
            if (preRevision.blockingCount === 0
            ...
            const [baselineState, baselineHooks] = await Promise.all([
                ...
                throw new Error(`Cannot revise chapter ${targetChapter} safely: baseline snapshot ${baselineChapter} is unavailable (${String(error)})`);
            });
            ...
            let stateValidation = await stateValidator.validate(revisedContent, targetChapter, baselineState, settledRevision.updatedState, baselineHooks, settledRevision.updatedHooks, language);
            ...
            const lengthWarnings = this.buildLengthWarnings(targetChapter, revisedCount, lengthSpec);
            const lengthTelemetry = this.buildLengthTelemetry({
                lengthSpec,
    ...
    // ---------------------------------------------------------------------------
    async writeNextChapter(bookId, wordCount, temperatureOverride, externalContext) {
    ...
    async writeChapters(bookId, chapterCount, options = {}) {
        ...
        try {
            ...
            for (let index = 0; index < chapterCount; index += 1) {
                this.throwIfOperationAborted();
                const result = await this._writeNextChapterLocked(bookId, options.wordCount, options.temperatureOverride, options.externalContext ?? this.config.externalContext);
                results.push(result);
    ...
    async resyncChapterStateAndAudit(bookId, chapterNumber, options = {}) {
        const releaseLock = await this.state.acquireBookLock(bookId);
        try {
    ...
    async _writeNextChapterLocked(bookId, wordCount, temperatureOverride, externalContext) {
        ...
        try {
            const result = await this._executeNextChapterLocked(bookId, wordCount, temperatureOverride, externalContext);
            ...
            const lengthSpec = result.lengthTelemetry ?? buildLengthSpec(wordCount ?? book.chapterWordCount, await this.resolveBookLanguage(book));
    ...
    async _executeNextChapterLocked(bookId, wordCount, temperatureOverride, externalContext) {
        this.throwIfOperationAborted();
        await this.state.ensureControlDocuments(bookId);
        const book = await this.state.loadBookConfig(bookId);
        const bookDir = this.state.bookDir(bookId);
    ...
    async _repairChapterStateLocked(bookId, chapterNumber) {
        ...
        const [oldState, oldHooks] = await Promise.all([
            ...
            throw new Error(`Cannot repair chapter ${targetChapter} safely: baseline snapshot ${baselineChapter} is unavailable (${String(error)})`);
        });
        const writer = new WriterAgent(this.agentCtxFor("writer", bookId));
    ...
    async _resyncChapterArtifactsLocked(bookId, chapterNumber, options = {}) {
        const book = await this.state.loadBookConfig(bookId);
        const bookDir = this.state.bookDir(bookId);
        const stageLanguage = await this.resolveBookLanguage(book);
        const index = [...(await this.state.loadChapterIndex(bookId))];
        ...
        const reducedControlInput = await this.createGovernedArtifacts(book, bookDir, targetChapter, this.config.externalContext, { reuseExistingIntentWhenContextMissing: true });
        const writer = new WriterAgent(this.agentCtxFor("writer", bookId));
...
${matrix}`,
    ...
    async createGovernedArtifacts(book, bookDir, chapterNumber, externalContext, options) {
        ...
        const composed = await composeGovernedChapter({
            ...
            referenceContextProvider: (request) => selectBookReferenceContext(this.config.projectRoot, book.id, request, (selectionRequest) => composer.selectReferenceSections(selectionRequest)),
