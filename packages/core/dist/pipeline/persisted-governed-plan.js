...
function renderPersistedPlanMarkdown(intent, memo, plannerInputs) {
    return [
...
function renderMemoMarkdown(memo) {
    return [
...
function extractMarkedBlock(markdown, name) {
...
function readOptionalField(markdown, label) {
    const value = readField(markdown, label);
    return value && isMeaningfulLegacyValue(value) ? value : undefined;
}
function readBooleanField(markdown, label) {
    const value = readField(markdown, label);
    if (!value)
        return undefined;
    if (/^(yes|true|是)$/i.test(value))
        return true;
    if (/^(no|false|否)$/i.test(value))
        return false;
    return undefined;
}
function readListSection(markdown, heading) {
...
async function loadLegacyIntentPlan(bookDir, chapterNumber) {
    let intentMarkdown;
    ...
    if (!rawGoal || !isMeaningfulLegacyValue(rawGoal))
        return null;
    const goal = rawGoal;
...
function isMeaningfulLegacyValue(value) {
    const normalized = value.trim();
    if (!normalized)
        return false;
    if (/^\(?not found\)?$/i.test(normalized))
        return false;
    if (/^(?:none|null|undefined|n\/a)$/i.test(normalized))
        return false;
