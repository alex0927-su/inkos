 ...
 */
export function lookupModel(serviceId, modelId) {
    ...
    for (const p of getAllEndpoints()) {
        ...
        if (hit)
            matches.push({ model: hit, providerId: p.id });
    }
    if (matches.length === 0)
        return undefined;
    matches.sort((a, b) => {
...
export function listEnabledModels(serviceId) {
    const provider = getEndpoint(serviceId);
    if (!provider)
        return [];
    return provider.models.filter((m) => m.enabled !== false);
}
export function isActiveTextModel(model) {
    ...
    if (model.capabilities?.text === false)
        return false;
    if (model.capabilities?.imageOutput === true && model.capabilities?.text !== true)
        return false;
    return true;
}
export function listActiveTextModels(serviceId) {
    const provider = getEndpoint(serviceId);
    if (!provider)
        return [];
