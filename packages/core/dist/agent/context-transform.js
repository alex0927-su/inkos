...
export function createBookContextTransform(bookId, projectRoot, options = {}) {
    if (bookId === null) {
    ...
    return async (messages) => {
        ...
        const body = "[以下是当前书籍的上下文压缩包，每次对话时自动从磁盘读取生成。请基于这些内容进行创作和判断；需要完整原文时再按文件读取。]" +
            hintBlock + "\n\n" +
 ...
 */
export function createInteractiveFilmContextTransform(projectId, projectRoot) {
    return async (messages) => {
...
function buildMarkdownFileIndex(content) {
    ...
    for (const rawLine of lines) {
        ...
        if (!heading)
            continue;
        headingCount += 1;
        if (selected.length < MAX_INDEX_HEADINGS_PER_FILE)
...
async function readTruthFiles(storyDir) {
    let files;
    try {
...
async function listTruthMarkdownFiles(storyDir) {
...
async function listNestedMarkdownFiles(storyDir, relativeDir) {
