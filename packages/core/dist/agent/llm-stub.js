...
export function isLlmStubEnabled() {
 ...
 */
export function stubAgentStream(model, context) {
    const stream = createAssistantMessageEventStream();
    const text = lastUserText(context);
    const proposed = alreadyProposed(context);
    ...
    const content = wantStructure
        ...
        : [{ type: "text", text: "好的。" }];
    const stopReason = wantStructure ? "toolUse" : "stop";
    const message = {
        role: "assistant",
        content: content,
        api: model.api,
 ...
 */
export function stubChatCompletion(messages, _model) {
