...
import { buildWorldAnchorDelta, buildAddVariableDelta, buildDefineEndingDelta, buildUpsertCharactersDelta, buildConnectChoiceDelta, buildRemoveNodeDelta, } from "../interactive-film/authoring-tools.js";
...
import { generateNodeImage, defaultNodeImageDeps } from "../interactive-film/node-image.js";
...
export function createSetWorldAnchorTool(projectRoot, projectId) {
    return {
...
export function createAddVariableTool(projectRoot, projectId) {
    return {
        name: "add_variable",
...
export function createDefineEndingTool(projectRoot, projectId) {
    return {
        name: "define_ending",
...
export function createUpsertCharactersTool(projectRoot, projectId) {
    return {
...
function defaultSubmitNode(client, model, activatedSkills) {
...
function defaultSubmitStructure(client, model, activatedSkills) {
...
function nodeSystemPrompt(language) {
    return language === "en" ? NODE_SYSTEM_EN : NODE_SYSTEM_ZH;
}
function graphUpdatedDetails(rev, promptId, extra = {}) {
    return {
        kind: "graph_updated",
        rev,
...
export function createFillNodeTool(projectRoot, projectId, deps, language = "zh") {
    return {
        name: "fill_node",
...
export function createReviseNodeTool(projectRoot, projectId, deps, language = "zh") {
    return {
        name: "revise_node",
...
export function filmLLMDepsFromClient(client, model, options = {}) {
    return {
...
export function createDraftStructureTool(projectRoot, projectId, deps, language = "zh") {
    return {
...
const ConnectChoiceParams = Type.Object({
    node: Type.Unsafe({ description: "the full StoryNode (with updated choices) to upsert" }),
});
export function createConnectChoiceTool(projectRoot, projectId) {
    return {
        name: "connect_choice",
...
export function createRemoveNodeTool(projectRoot, projectId) {
    return {
        name: "remove_node",
...
export function createGenerateNodeImageTool(projectRoot, projectId, deps) {
    return {
        ...
        async execute(_id, params) {
            const graph = await loadStoryGraph(projectRoot, projectId);
            if (!graph)
                throw new Error(`interactive-film project ${projectId} has no story graph`);
 ...
 */
export function buildFilmAuthoringToolNames(confirmedIntent) {
    if (confirmedIntent === "draft_structure")
        return ["draft_structure"];
    if (confirmedIntent === "connect_choice")
        return ["connect_choice"];
    if (confirmedIntent === "remove_node")
        return ["remove_node"];
...
//# sourceMappingURL=film-authoring-tools.js.map