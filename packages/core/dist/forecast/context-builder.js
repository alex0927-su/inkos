...
const FINGERPRINT_FIXED_INPUTS = [
    "book.json",
...
const FINGERPRINT_ROLE_DIRS = ["主要角色", "次要角色", "major", "minor"];
/**
 ...
 */
async function collectFingerprintFiles(bookDir) {
    const [fixedEntries, stateFiles, roleEntryGroups] = await Promise.all([
        Promise.all(FINGERPRINT_FIXED_INPUTS.map(async (relPath) => {
            ...
            return content === null ? null : [relPath, content];
        })),
        ...
        Promise.all(FINGERPRINT_ROLE_DIRS.map(async (tier) => {
            ...
            return Promise.all(names.map(async (name) => [`story/roles/${tier}/${name}`, await readOrEmpty(join(dir, name))]));
        })),
    ]);
    return [
        ...fixedEntries.filter((entry) => entry !== null),
        ...stateFiles.map(({ name, content }) => [`story/state/${name}`, content]),
...
export function renderForecastContextMarkdown(context) {
 ...
 */
async function resolveBaseChapter(bookDir) {
    let files;
    try {
...
async function readStateFiles(bookDir) {
