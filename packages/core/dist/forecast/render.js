...
export function renderForecastComparisonMarkdown(forecast) {
    ...
    const tableRows = forecast.branches.map((branch) => `| ${branch.branchId} | ${escapeCell(branch.title)} | ${branch.intentAlignment.score} | ${branch.risks.length} | ${escapeCell(branch.premise)} |`);
...
function renderBranchSection(branch, zh, options = {}) {
...
function joinOrNone(items, none) {
