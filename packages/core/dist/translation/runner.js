...
import { commitProductionArtifacts, createProductionRunSnapshot, writeProductionRunSnapshot, } from "../production/harness.js";
import { loadTranslationChapter, loadTranslationGlossary, loadTranslationManifest, mergeGlossaryTerms, saveTranslationProgress, saveTranslationManifest, } from "./run-store.js";
export async function runTranslationProject(projectRoot, projectId, options) {
...
function orderedTranslatedSegments(sourceSegments, translatedByIndex) {
    return sourceSegments.map((segment) => translatedByIndex.get(segment.index) ?? segment);
}
function updateChapterStatus(manifest, chapterNumber, status) {
    return {
        ...manifest,
        updatedAt: new Date().toISOString(),
        chapters: manifest.chapters.map((chapter) => chapter.number === chapterNumber ? { ...chapter, status } : chapter),
    };
}
