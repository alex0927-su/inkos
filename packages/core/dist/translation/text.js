...
export function splitTranslationChapters(text) {
...
export function segmentTranslationText(text, maxChars = 1200) {
    const normalized = normalizeTranslationText(text);
    if (!normalized)
        return [];
    ...
    return units.flatMap((paragraph) => paragraph.length > maxChars ? splitLongParagraph(paragraph, maxChars) : [paragraph]);
}
function splitMarkdownHeadings(text) {
    const lines = text.split("\n");
    const headings = [];
    for (let i = 0; i < lines.length; i++) {
        const match = /^#{1,3}\s+(.+?)\s*$/.exec(lines[i] ?? "");
        if (match)
            headings.push({ title: match[1].trim(), line: i });
    }
...
function packBoundaryUnits(units, maxChars) {
    const chunks = [];
    let current = "";
