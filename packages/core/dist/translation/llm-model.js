    ...
    return {
        ...
        async reviewChapter(request) {
            ...
            return {
                ...
                issues: Array.isArray(parsed.issues) ? parsed.issues.filter((issue) => typeof issue === "string") : [],
            };
        },
    };
}
function parseTranslatedSegments(value, sourceSegments) {
    ...
    const parsed = value.flatMap((item) => {
        ...
        if (!Number.isInteger(index) || !sourceIndex.has(index) || !target)
            return [];
        return [{
                index,
                target,
    ...
    if (parsed.length === 0)
        throw new Error("Translation model returned no usable translated segments.");
    return parsed;
}
...
//# sourceMappingURL=llm-model.js.map