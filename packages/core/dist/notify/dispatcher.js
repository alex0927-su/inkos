                ...
                case "telegram":
                    {
                        ...
                        await sendTelegram({ botToken: channel.botToken, chatId: channel.chatId }, channel.format === "text" ? plainText : markdownText, channel.format);
                    }
                    break;
                case "feishu":
                    {
                        const { sendFeishu } = await import("./feishu.js");
                        await sendFeishu({ webhookUrl: channel.webhookUrl }, message.title, channel.format === "text" ? plainBody : message.body, channel.format);
                    }
                    break;
                case "wechat-work":
                    {
                        const { sendWechatWork } = await import("./wechat-work.js");
                        await sendWechatWork({ webhookUrl: channel.webhookUrl }, channel.format === "text" ? plainText : markdownText, channel.format);
                    }
                    break;
                case "webhook":
