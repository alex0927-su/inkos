...
export function assertSafeTruthFileName(fileName) {
    ...
    if (SAFE_TRUTH_OUTLINE_FILE_NAMES.has(lower))
        return lower;
    if (SAFE_ROLE_TRUTH_FILE_RE.test(withExtension))
...
async function withBookMutationLock(state, bookId, task) {
    ...
    return {
        ...
        chat: async (input, options) => {
            ...
            if (instrumentedPipeline.config?.client && instrumentedPipeline.config?.model) {
                try {
                    response = await runWorkerAgent(instrumentedPipeline.config.client, instrumentedPipeline.config.model, [
                        {
                            role: "system",
                            content: [
