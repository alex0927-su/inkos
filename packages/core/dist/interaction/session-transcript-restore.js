...
const RESTORED_HISTORY_BOUNDARY_ZH = "[已完成的历史上下文]\n" +
    "以上是已经完成并提交的历史上下文，只能用于回忆，不代表当前轮已经执行了动作。\n" +
    "当前轮必须优先遵循用户接下来输入的最新指令；如果最新输入是提问、讨论或确认，直接回答，不要因为历史工具结果跳过判断或继续执行旧动作。";
const RESTORED_HISTORY_BOUNDARY_EN = "[Completed history context]\n" +
...
export function appendRestoredHistoryBoundary(messages, language) {
    ...
    return [
        ...messages,
        systemMessage(language === "zh" ? RESTORED_HISTORY_BOUNDARY_ZH : RESTORED_HISTORY_BOUNDARY_EN, timestamp + 1),
    ];
}
...
export function adaptRestoredAgentMessagesForModel(messages, target) {
    const adapted = [];
    const pushToolResultsAsSystem = (toolResults) => {
...
export function committedMessageEvents(events, sessionKind) {
    const requestKinds = new Map(events
...
function requestKindMap(events) {
    return new Map(events
        .filter((event) => event.type === "request_started")
        .map((event) => [event.requestId, event.sessionKind]));
}
function textOnlyAgentMessage(event) {
    const raw = event.message;
    if (!isObject(raw) || event.role === "toolResult")
        return null;
    if (event.role === "user" || event.role === "system") {
        const content = textFromContent(raw.content);
        return content ? { role: event.role, content, timestamp: event.timestamp } : null;
    }
    if (event.role === "assistant") {
        const textBlocks = contentBlocks(raw).filter((block) => isObject(block) &&
            block.type === "text" &&
            typeof block.text === "string" &&
            block.text.trim().length > 0);
...
function buildHistoricalToolSummary(events, sessionKind) {
...
function requestIdsWithToolActivity(events) {
    const ids = new Set();
...
function requestIdsUsingSkill(events) {
    const ids = new Set();
    for (const event of events) {
        if (event.role !== "assistant")
            continue;
        ...
        if (!isObject(raw))
            continue;
        if (contentBlocks(raw).some((block) => (isObject(block)
            && block.type === "toolCall"
...
export async function restoreAgentMessagesFromTranscript(projectRoot, sessionId, sessionKind) {
...
function messageEventToInteractionMessage(event, restoredToolExecutions, restoredThinking, options = {}) {
    const raw = event.message;
    if (!isObject(raw))
        return null;
    if (event.role === "toolResult")
    ...
    if (event.role === "assistant") {
        ...
        if (!content && !thinking && !toolExecutions?.length)
            return null;
        if (!content && !toolExecutions?.length)
            return null;
        return {
            role: "assistant",
            content,
...
function messageEventsToInteractionMessages(events) {
    ...
    const flushPendingToolExecutions = () => {
        ...
        if (last?.role === "assistant") {
            ...
            messages.push({
                ...
                timestamp: pendingToolExecutions.reduce((latest, execution) => Math.max(latest, execution.completedAt ?? execution.startedAt), 0),
            });
        }
        clearPending();
    };
    ...
    const hasCompletedPlayTool = (executions) => executions.some((execution) => execution.status === "completed"
    ...
    for (const event of events) {
        ...
        if (event.role === "assistant" && isObject(raw)) {
            ...
            const legacyToolExecutions = event.legacyDisplay?.toolExecutions;
...
export async function deriveBookSessionFromTranscript(projectRoot, sessionId) {
    ...
    let playMode = created?.type === "session_created" ? created.playMode : undefined;
    ...
    for (const event of events) {
        ...
        if ("bookId" in event && event.bookId !== undefined)
            bookId = event.bookId;
        if ("sessionKind" in event && event.sessionKind !== undefined)
            sessionKind = event.sessionKind;
        if ("playMode" in event && event.playMode !== undefined)
