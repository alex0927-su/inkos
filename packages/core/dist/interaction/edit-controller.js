...
function assertEntityRenameTargetIsSafe(newValue) {
    if (/[/\\]/.test(newValue)) {
...
async function planEntityFileRenames(root, files, oldValue, newValue) {
    const planned = [];
    for (const filePath of files) {
        ...
        if (targetExists) {
            throw new Error(`Cannot rename "${relative(root, filePath)}" to "${nextBase}": a file with that name already exists.`);
        }
...
async function findChapterPath(root, chapterNumber) {
...
async function clearChapterRuntimeFiles(root, chapterNumber) {
    const paddedChapter = String(chapterNumber).padStart(4, "0");
...
function markChapterForManualReview(index, chapterNumber, issue, wordCount) {
...
function roughChapterLength(content) {
    return content
        .replace(/^---[\s\S]*?---\s*/m, "")
        .replace(/^#{1,6}\s+.*$/gm, "")
        .replace(/\s+/g, "")
...
async function executeChapterReplace(deps, request) {
    const root = deps.bookDir(request.bookId);
    ...
    const updatedIndex = markChapterForManualReview(await deps.loadChapterIndex(request.bookId), request.chapterNumber, "Manual chapter replacement requires review before continuation.", roughChapterLength(fullText));
...
async function executeChapterLocalEdit(deps, request) {
    ...
    const removedRuntimeFiles = await clearChapterRuntimeFiles(root, request.chapterNumber);
    const updatedIndex = markChapterForManualReview(await deps.loadChapterIndex(request.bookId), request.chapterNumber, "Manual text edit requires review before continuation.", roughChapterLength(nextContent));
    await deps.saveChapterIndex(request.bookId, updatedIndex);
    return {
        transactionType: request.kind,
        bookId: request.bookId,
...
function replaceChapterTargetText(content, targetText, replacementText) {
...
function flexibleWhitespacePattern(targetText) {
...
function replaceApproximateParagraph(content, targetText, replacementText) {
    ...
    for (const match of content.matchAll(paragraphPattern)) {
        ...
        if (normalized.length < 24)
            continue;
        if (normalized.length < target.length * 0.35 || normalized.length > target.length * 3)
            continue;
...
function normalizeApproximateText(text) {
    return text
        .normalize("NFKC")
        .toLowerCase()
        .replace(/[^\p{L}\p{N}]+/gu, "");
}
function approximateTextScore(target, candidate) {
