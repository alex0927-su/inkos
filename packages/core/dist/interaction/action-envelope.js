import { z } from "zod";
import { PlayModeSchema } from "./session.js";
import { StoryNodeSchema } from "../interactive-film/graph-schema.js";
import { SHORT_FICTION_EN_MAX_WORDS_PER_CHAPTER, SHORT_FICTION_EN_MIN_WORDS_PER_CHAPTER, SHORT_FICTION_MAX_CHARS_PER_CHAPTER, SHORT_FICTION_MIN_CHARS_PER_CHAPTER, } from "../agents/short-fiction.js";
...
export function shortRunCharsPerChapterRange(language) {
    return language === "en"
...
export function shortRunCharsPerChapterError(value, language) {
...
export const ShortRunActionPayloadSchema = z.object({
    ...
    if (payload.language === undefined || payload.charsPerChapter === undefined)
        return;
    const { min, max } = shortRunCharsPerChapterRange(payload.language);
    if (payload.charsPerChapter >= min && payload.charsPerChapter <= max)
        return;
    ctx.addIssue({
...
export const FanficCreateActionPayloadSchema = z.object({
    ...
    targetChapters: z.number().int().min(1).optional(),
    chapterWordCount: z.number().int().min(1).optional(),
}).strict().refine((payload) => Boolean(payload.sourceText?.trim() || payload.sourcePath?.trim()), { message: "fanficCreate requires sourceText or sourcePath" });
...
export const ImitationCreateActionPayloadSchema = z.object({
    ...
    targetChapters: z.number().int().min(1).optional(),
    chapterWordCount: z.number().int().min(1).optional(),
}).strict().refine((payload) => Boolean(payload.referenceText?.trim() || payload.referencePath?.trim()), { message: "imitationCreate requires referenceText or referencePath" });
...
export function normalizeSkillIdList(value) {
...
export function normalizeActionSource(value) {
    if (value === undefined || value === null || value === "")
        return "free-text";
    return ActionSourceSchema.parse(value);
}
export function normalizeRequestedIntent(value) {
    if (value === undefined || value === null || value === "")
        return undefined;
