 ...
 */
export function analyzeStyle(text, sourceName, language = "zh") {
    ...
    const measure = (s) => isEn ? (s.match(/[A-Za-z0-9]+(?:'[A-Za-z0-9]+)?/g)?.length ?? 0) : s.replace(/\s+/g, "").length;
