...
import { composeCurrentArcProse, extractCollaboratorRows, extractOpponentRows, extractProtagonistRow, formatRelevantThreads, formatRecentSummaries, formatRecyclableHooks, readBookRules, readCharacterMatrix, readEmotionalArcs, readSubplotBoard, } from "./planner-context.js";
...
export class PlannerAgent extends BaseAgent {
    ...
    async planChapter(input) {
        ...
        const activeHookCount = memorySelection.activeHooks.filter((hook) => hook.status !== "resolved" && hook.status !== "deferred").length;
        ...
        const lengthSpec = buildLengthSpec(input.book.chapterWordCount, input.book.language ?? "zh");
