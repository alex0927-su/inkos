...
import { filterSummaries, filterSubplots, filterEmotionalArcs, } from "../utils/context-filter.js";
...
import { buildRuntimeStateArtifacts, buildRuntimeStateArtifactsFromSnapshot, loadRuntimeStateSnapshotAtChapter, } from "../state/runtime-state-store.js";
...
export class WriterAgent extends BaseAgent {
    ...
    async writeChapter(input) {
        ...
        const [volumeOutline, styleGuide, currentState, ledger, hooks, chapterSummaries, subplotBoard, emotionalArcs, characterMatrix, styleProfileRaw, fanficCanonRaw,] = await Promise.all([
        ...
        const creativeSystemPrompt = await this.withPromptPackGuidance(buildWriterSystemPrompt(book, genreProfile, bookRules, bookRulesBody, genreBody, styleGuide, styleFingerprint, chapterNumber, "creative", fanficContext, resolvedLanguage, "governed", resolvedLengthSpec), "longform.writer");
    ...
    async settleChapterState(input) {
        ...
        const settlement = settleResult.settlement;
        const runtimeStateArtifacts = await this.buildRuntimeStateArtifactsIfPresent(input.bookDir, settlement.runtimeStateDelta, resolvedLanguage, input.chapterNumber, input.allowReapply, input.baselineChapter, input.allowNewHooks);
        return {
            chapterNumber: input.chapterNumber,
            title: input.title,
            content: input.content,
    ...
    buildGovernedUserPrompt(params) {
        ...
        const directionEntries = params.contextPackage.selectedContext.filter((entry) => DIRECTION_SOURCES.has(entry.source));
        const otherEntries = params.contextPackage.selectedContext.filter((entry) => !DIRECTION_SOURCES.has(entry.source));
...
- 允许区间：${lengthSpec.softMin}-${lengthSpec.softMax}字`;
    ...
    async buildRuntimeStateArtifactsIfPresent(bookDir, delta, language, authoritativeChapterNumber, allowReapply, baselineChapter, allowNewHooks) {
        if (!delta)
            return null;
    ...
    async renderAppendedChapterSummary(bookDir, summary, language) {
