...
export async function runResearchReport(input, deps = {}) {
    ...
    const claims = sources.map((source) => ({
    ...
    const unknowns = sources.length === 0
        ...
        : partialFailures.length > 0
            ...
            : [];
    const confidence = sources.length >= 3 && partialFailures.length === 0
        ? "high"
        : sources.length >= 1
            ? "medium"
            : "low";
...
function depthConfig(depth) {
    if (depth === "deep")
        return { queryCount: 3, maxResults: 5, fetchCount: 6 };
    if (depth === "standard")
        return { queryCount: 2, maxResults: 4, fetchCount: 4 };
    return { queryCount: 1, maxResults: 3, fetchCount: 2 };
}
function buildQueries(topic, purpose, depth) {
    const hint = PURPOSE_HINTS[purpose];
    const queries = [`${topic} ${hint}`];
    if (depth !== "quick")
        queries.push(`${topic} 资料 来源`);
    if (depth === "deep")
        queries.push(`${topic} 争议 误区 核查`);
    return queries;
}
...
function buildCreativeImplications(purpose, sourceCount) {
    if (sourceCount === 0)
        return ["Do not promote any collected item to hard story canon yet."];
    if (purpose === "profession")
        return ["Use workflow details and terminology as texture, but keep hard claims source-backed."];
    if (purpose === "era")
        return ["Use era constraints to police anachronisms in scenes, props, dialogue, and institutions."];
    if (purpose === "worldbuilding")
        return ["Convert verified social, material, and institutional details into scene rules rather than exposition dumps."];
    if (purpose === "market")
        return ["Treat market observations as positioning hints, not as story canon."];
    if (purpose === "fact-check")
        return ["Facts with only one source should remain soft until cross-checked."];
    return ["Use sourced details as references; unresolved points should stay out of hard canon."];
}
function renderResearchMarkdown(topic, input, report) {
    return [
        `# Research: ${topic}`,
        "",
