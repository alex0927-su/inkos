...
class LongFormProductionAgent extends BaseAgent {
    async recoverProductionMarkdown(fragments, language, requiredHeadings) {
...
export class ScriptCreationAgent extends LongFormProductionAgent {
    ...
    async writeScript(input) {
        ...
        const response = await completeLongForm({
            ...
            recoverAfterContinuation: (fragments) => this.recoverProductionMarkdown(fragments, language, language === "en" ? ["## Characters", "## Script"] : ["## 人物", "## 剧本正文"]),
        });
...
export class StoryboardCreationAgent extends LongFormProductionAgent {
    ...
    async writeStoryboard(input) {
        ...
        const response = await completeLongForm({
            ...
            recoverAfterContinuation: (fragments) => this.recoverProductionMarkdown(fragments, language, language === "en" ? ["## Storyboard", "## Image Prompts"] : ["## 分镜表", "## 图像提示词"]),
        });
        return extractProductionDocument(response.content, input.title);
    }
}
export class InteractiveFilmCreationAgent extends LongFormProductionAgent {
    ...
    async writeInteractiveFilm(input) {
        const language = input.language ?? "zh";
        const messages = [
        ...
        const response = await completeLongForm({
            ...
            recoverAfterContinuation: (fragments) => this.recoverProductionMarkdown(fragments, language, language === "en"
...
export function renderScriptSpec(input) {
...
export function renderStoryboardSpec(input) {
    if ((input.language ?? "zh") === "en") {
        return [
...
export function renderInteractiveFilmSpec(input) {
    if ((input.language ?? "zh") === "en") {
        return [
...
export function extractStoryboardImagePrompts(raw) {
...
export function extractMarkdownSection(raw, headings) {
...
export function countMarkdownSections(raw, headings) {
    const normalizedHeadings = headings.map(normalizeHeadingText);
    let count = 0;
...
export function extractProductionDocument(raw, title) {
    const lines = raw.split(/\r?\n/);
...
function headingMatches(text, heading) {
    if (text === heading)
        return true;
...
export function normalizeScriptEpisodeEndLabels(script) {
    const lines = script.split(/\r?\n/);
    let currentEpisode = null;
    return lines.map((line) => {
        const heading = /^#{1,6}\s*第\s*([一二三四五六七八九十百千万\d]+)\s*集(?:\s|$)/u.exec(line.trim());
        if (heading)
            currentEpisode = heading[1];
        if (!currentEpisode)
            return line;
        return line.replace(/(字幕\s*[：:]\s*)第\s*[一二三四五六七八九十百千万\d]+\s*集完/gu, `$1第${currentEpisode}集完`);
    }).join("\n");
}
function buildScriptCreationSystemPrompt(language = "zh") {
    if (language === "en") {
        return [
...
function buildScriptCreationUserPrompt(input, language = "zh") {
    if (language === "en") {
        return [
            "## Creation Spec",
...
function buildStoryboardCreationSystemPrompt(language = "zh") {
    if (language === "en") {
        return [
...
function buildStoryboardCreationUserPrompt(input, language = "zh") {
...
function buildInteractiveFilmCreationSystemPrompt(language = "zh") {
    if (language === "en") {
        return [
...
function buildInteractiveFilmCreationUserPrompt(input, language = "zh") {
    if (language === "en") {
        return [
...
function formatScriptTarget(value, language = "zh") {
    if (language === "en") {
        switch (value) {
...
function summarizeSourceForSpec(sourceText, language = "zh") {
    const text = sourceText?.replace(/\s+/g, " ").trim();
    if (language === "en") {
        if (!text)
            return "No full source material provided.";
...
function estimateInteractiveFilmMaxTokens(input) {
    const episodes = input.episodeCount ?? 6;
    return Math.min(36000, Math.max(16000, episodes * 3000));
}
function extractPromptLines(markdown) {
    const prompts = [];
...
//# sourceMappingURL=script-storyboard.js.map