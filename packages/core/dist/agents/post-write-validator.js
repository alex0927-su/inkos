...
export function validatePostWrite(content, genreProfile, bookRules, languageOverride) {
    ...
    if (personViolation)
        violations.push(personViolation);
    return violations;
}
function detectNarrativePersonDrift(content, bookRules) {
    if (bookRules?.narrativePerson !== "first")
        return null;
...
function detectFirstPersonInnerStateSlip(content) {
    const sentences = content
        .split(/(?<=[。！？!?])|\n+/)
