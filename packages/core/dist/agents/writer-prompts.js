...
// Public API
// ---------------------------------------------------------------------------
export function buildWriterSystemPrompt(book, genreProfile, bookRules, bookRulesBody, genreBody, styleGuide, styleFingerprint, chapterNumber, mode = "full", fanficContext, languageOverride, inputProfile = "legacy", lengthSpec) {
...
function buildGovernedInputContract(language, governed) {
    if (!governed)
        return "";
    if (language === "en") {
...
// ---------------------------------------------------------------------------
function buildChapterMemoContract(language, governed) {
    if (!governed)
        return "";
    if (language === "en") {
...
function buildLengthGuidance(lengthSpec, language) {
    if (language === "en") {
...
// ---------------------------------------------------------------------------
export function buildGoldenOpeningDiscipline(chapterNumber, language) {
...
function buildNarrativePersonRule(bookRules, language) {
...
// ---------------------------------------------------------------------------
function buildCreativeOutputFormat(book, gp, lengthSpec) {
...
// Output format
// ---------------------------------------------------------------------------
function buildOutputFormat(book, gp, lengthSpec) {
    const resourceRow = gp.numericalSystem
        ? "| 当前资源总量 | X | 与账本一致 |\n| 本章预计增量 | +X（来源） | 无增量写+0 |"
        : "";
    const preWriteTable = `=== PRE_WRITE_CHECK ===
...
// ---------------------------------------------------------------------------
function buildEnglishPreWriteTable(gp) {
    const resourceRow = gp.numericalSystem
...
function buildEnglishContentBlocks(lengthSpec) {
...
function buildEnglishCreativeOutputFormat(_book, gp, lengthSpec) {
...
function buildEnglishOutputFormat(_book, gp, lengthSpec) {
    const postSettlement = gp.numericalSystem
        ? `=== POST_SETTLEMENT ===
