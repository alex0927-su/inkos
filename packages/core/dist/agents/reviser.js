...
import { buildGovernedCharacterMatrixWorkingSet, buildGovernedHookWorkingSet, } from "../utils/governed-working-set.js";
...
${chapterContent}`;
        ...
        const output = this.parseOutput(response.content, mode, chapterContent, autoOutputMode);
    ...
    parseOutput(content, mode, originalChapter, autoOutputMode = "allow-full") {
        const extract = (tag) => {
...
function resolveAutoOutputMode(issues) {
    ...
    if (scopedBlocking.length > 0) {
        ...
        if (scopedBlocking.length === issues.filter((issue) => issue.severity !== "info").length
