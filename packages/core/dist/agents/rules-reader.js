...
export async function readBookRules(bookDir) {
    ...
    if (storyFrameRaw) {
        ...
        if (frontmatterMatch) {
            ...
            const parsed = tryParseBookRulesFrontmatter(frontmatterMatch[1], (err) => {
                ...
                console.warn(`[rules-reader] story_frame.md frontmatter is malformed at ${bookDir}/story/outline/story_frame.md — falling back to legacy book_rules.md. Error: ${err instanceof Error ? err.message : String(err)}`);
            });
            if (parsed)
                return parsed;
    ...
    if (rulesRaw) {
        // eslint-disable-next-line no-console
        console.warn(`[rules-reader] book_rules.md at ${bookDir}/story/book_rules.md is a compat shim and no legacy story_frame frontmatter was parseable — returning null instead of silently zeroing out rules.`);
    }
    return null;
}
