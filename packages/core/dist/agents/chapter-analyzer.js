...
export class ChapterAnalyzerAgent extends BaseAgent {
    ...
    async analyzeChapter(input) {
        ...
        const [currentState, ledger, hooks, subplotBoard, emotionalArcs, characterMatrix, storyBible, volumeOutline,] = await Promise.all([
        ...
        const chapterSummaries = this.renderSummarySnapshot(memorySelection.summaries, resolvedLanguage);
        ...
        const reducedControlBlock = governedMode && input.chapterIntent && input.contextPackage && input.ruleStack
            ...
            : "";
        const systemPrompt = this.buildSystemPrompt(book, genreProfile, genreBody, bookRulesBody, resolvedLanguage);
        const userPrompt = this.buildUserPrompt({
            ...
            bibleBlock: !governedMode && storyBible !== this.missingFilePlaceholder(resolvedLanguage)
                ? resolvedLanguage === "en"
                    ...
                    : `\n## 世界观设定\n${storyBible}\n`
                : "",
            outlineOrControlBlock: reducedControlBlock || (volumeOutline !== this.missingFilePlaceholder(resolvedLanguage)
                ? resolvedLanguage === "en"
                    ? `\n## Volume Outline\n${volumeOutline}\n`
                    : `\n## 卷纲\n${volumeOutline}\n`
                : ""),
    ...
    buildSystemPrompt(book, genreProfile, genreBody, bookRulesBody, language) {
        if (language === "en") {
...
${overrides}\n`;
    ...
    renderSummarySnapshot(summaries, language) {
    ...
    async readFileOrDefault(path, language) {
        try {
    ...
    missingFilePlaceholder(language) {
